...
public final class VaporizationManager {
    ...
    public static void tick(ServerLevel level, LargeCauldronBlockEntity cauldron) {
    ...
    private static void processFirstSource(VaporizationContext context) {
        ...
        for (VaporizationSource source : VaporizationSources.getSources()) {
            ...
            FluidStack simulated = context.cauldron().getFluids().drainStoredFluid(
                request,
                IFluidHandler.FluidAction.SIMULATE
            );
            if (!FluidStack.matches(simulated, request)) continue;
            FluidStack drained = context.cauldron().getFluids().drainStoredFluid(
                request,
