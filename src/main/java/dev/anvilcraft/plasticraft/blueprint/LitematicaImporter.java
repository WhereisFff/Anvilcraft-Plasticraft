...
public final class LitematicaImporter {
    ...
    public static ConvertedStructure convert(CompoundTag litematic) throws ConstructionBlueprintException {
        if (!litematic.contains("Regions", Tag.TAG_COMPOUND)) {
        ...
        for (String regionName : regions.getAllKeys().stream().sorted().toList()) {
            parsedRegions.add(parseRegion(regionName, regions.getCompound(regionName), warnings));
        }
        ...
        for (ParsedRegion region : parsedRegions) {
            ...
            for (int index = 0; index < region.palette().size(); index++) {
                paletteMap[index] = internState(paletteOut, region.palette().getCompound(index));
            }
        ...
        out.putInt("DataVersion", litematic.getInt("MinecraftDataVersion"));
    ...
    private record ParsedRegion(
        ...
        int blit(
            ...
            for (int index = 0; index < this.tileEntities.size(); index++) {
                CompoundTag tileEntity = this.tileEntities.getCompound(index).copy();
                int localX = tileEntity.getInt("x");
                int localY = tileEntity.getInt("y");
                int localZ = tileEntity.getInt("z");
    ...
    private static ParsedRegion parseRegion(
        ...
        if (!region.contains("Position", Tag.TAG_COMPOUND) || !region.contains("Size", Tag.TAG_COMPOUND)) {
            throw new ConstructionBlueprintException("corrupt_litematic", "region " + name + " missing bounds");
        }
        BlockPos position = readVec(region.getCompound("Position"));
        BlockPos rawSize = readVec(region.getCompound("Size"));
        ...
        for (String key : region.getAllKeys()) {
        ...
        return new ParsedRegion(
            ...
            bitsPerEntry,
            region.getList("TileEntities", Tag.TAG_COMPOUND),
            region.getList("Entities", Tag.TAG_COMPOUND)
        );
    }
