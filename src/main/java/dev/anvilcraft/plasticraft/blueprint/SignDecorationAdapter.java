...
public final class SignDecorationAdapter {
    ...
    private static void extractSide(
        ...
        if (!config.contains(key, Tag.TAG_COMPOUND)) return;
        CompoundTag side = config.getCompound(key);
        ...
        if (text.getColor() != DyeColor.BLACK) {
            ...
            decorations.add(new Decoration(
                ...
                new ItemStack(DyeItem.byColor(text.getColor())),
                payload
            ));
        }
    ...
    private static void composeSide(
        ...
        if (color != null) {
            side.putString(COLOR, color.getString(COLOR));
        }
        if (glowing) {
    ...
    private static void stripPendingSide(
        ...
        if (!result.contains(key, Tag.TAG_COMPOUND)) return;
        CompoundTag side = result.getCompound(key).copy();
