...
public final class IgnitionBuildAdapter {
    ...
    public static boolean canDeliver(WorkingAllayEntity worker, ConstructionBuildOp op) {
        ...
        return supports(op.target()) ? ignition : !ignition;
    }
    ...
    private static boolean commitPortal(ServerLevel level, List<ConstructionBuildOp> group, ConstructionBuildOp core) {
        ...
        for (ConstructionBuildOp op : group) {
            ...
            BlockState present = level.getBlockState(op.pos());
            if (!present.isAir() && !present.is(Blocks.FIRE) && !present.is(Blocks.NETHER_PORTAL)) return false;
        }
        int width = max - min + 1;
        int height = maxY - minY + 1;
        if (width < 2 || width > 21 || height < 3 || height > 21 || group.size() != width * height) return false;
        ...
        for (int x = 0; x < width; x++) {
            if (!isFrame(level, bottom.relative(along, x).below())
                || !isFrame(level, bottom.relative(along, x).above(height))) return false;
        }
        for (int y = 0; y < height; y++) {
            if (!isFrame(level, bottom.relative(along, -1).above(y))
                || !isFrame(level, bottom.relative(along, width).above(y))) return false;
        }
        PortalShape shape = new PortalShape(level, bottom, axis);
    ...
    private static boolean isFrame(ServerLevel level, BlockPos pos) {
        return level.hasChunkAt(pos) && level.getBlockState(pos).isPortalFrame(level, pos);
    }
}
