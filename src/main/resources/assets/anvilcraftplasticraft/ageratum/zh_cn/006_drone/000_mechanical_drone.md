...
- 建设无人机发现范围是到最近可执行施工目标的直线距离 128 格，触及距离 1 格；从所有者背包取料飞到接近位交付施工投影或放置临时流体封堵块，再飞回玩家；全部交付完成后先飞离蓝图范围再降落，不在工地原地落下；所有者在工地外时飞回其身边；取料与返还不计瞬时费，每次交付或封堵扣除 2,560 FE
- 拆除无人机发现范围同样是到最近可拆目标 128 格，触及距离 1 格；到位后按普通铁砧砸切石机上方方块的语义瞬间拆除，每次扣除 2,560 FE，不吸取掉落物；基岩等负硬度方块视为永久障碍并跳过对应建造，不会阻塞整个拆除阶段；缺少拆除无人机时按缺工种策略暂停等待或跳过可拆障碍
- 收集无人机自由模式每次吸入后以当前位置重扫 16 格内任意掉落物，触及 1 格，每次吸入扣除 2,560 FE，不吸经验球，磁铁无耐久损耗；九格库存装满后降落不再吸，所有者走进 16 格按原版背包规则卸货，塞不下的留在机内；任务模式只收带本任务标记的掉落，附近未标记物留给自由模式、玩家或漏斗；任务清场结束或满载后先飞离蓝图范围再降落，不在工地原地落下；没有收集机时拆除结束后不阻塞，掉落留在世界并继续建造
- 已交付的施工投影在世界格仍是空气、没有方块实体，但外观与该方块在世界中一致（木头就是不透明木板，水就是半透明水面），并提供真实碰撞，可以站上去；栅栏、墙和门按成品邻接连接；未交付全息可以穿过；空碰撞目标（如红石粉）不产生假碰撞，已交付和提交后的原版红石粉按蓝图外观与功率整区静默写入，铁砧工艺红石导线还会把蓝图四向写入本体端口覆盖表，之后网络更新也不会按邻居补臂或把平行线连成一片；提交后功率可随世界供电变化，连接方向保持蓝图；方块实体内容真实扣料，大型多方块只消耗一份核心；桶装流体交付后返还空桶，树脂块捕获先返还 1–3 个树脂再领下一项，树脂铁砧整砧消耗；塑料实体施工期只显示投影、提交后才生成硬碰撞，六向都被未交付方块占住时无人机会进入目标格交付
