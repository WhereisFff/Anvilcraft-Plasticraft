...
- 收集无人机自由模式每次吸入后以当前位置重扫 16 格内任意掉落物，触及 1 格，每次吸入扣除 2,560 FE，不吸经验球，磁铁无耐久损耗；九格库存装满后降落不再吸，所有者走进 16 格按原版背包规则卸货，塞不下的留在机内；任务模式只收带本任务标记的掉落，附近未标记物留给自由模式、玩家或漏斗；没有收集机时拆除结束后不阻塞，掉落留在世界并继续建造
