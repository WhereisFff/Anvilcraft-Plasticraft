...
硬化树脂炼药锅是可移动、受重力和浮力影响的实体炼药锅，可附着在六个方向。放置时，被点击面由两条对角线分成四个区域，准星所在区域靠近的边决定面内朝向，因此可以直接放出全部 24 种姿态。手持制品瞄准方块时，白色粗线显示面框与两条对角线，整个面以半透明白色与透明网格交错，当前分区的三条边和其中的白色格变为绿色；切换分区时，绿色边线和网格会平滑滑向新区域，瞄向另一方块面时整张预览也会平滑移到新位置。它能保存物品和最多一桶流体，并作为铁砧工艺配方中的锅使用；Jade 会显示其中保存的物品和流体。
