...
| <ref item="minecraft:flint_and_steel"/> or <ref item="minecraft:fire_charge"/> | Reach 1 block; only ignites blueprint fire, soul fire and Nether portals, without carrying materials, building, sealing, demolishing or collecting |

Tools given directly by a player are fixed equipment: the allay keeps its specialization. Empty-handed workers on a lounge job can borrow useful tools from the warehouse below: crab claws for building, stonecutters for demolition, magnets for collection, spyglasses for chunk loading and flint and steel for ignition, falling back to fire charges when flint and steel is unavailable

Automatic equipment fills current shortages while retaining useful workers. Observation gets enough workers for missing coverage windows; collection prefers 1 magnet worker and ignition waits for ready supports or frames before assigning 1 dedicated worker holding flint and steel or a fire charge. Separate workers carry materials and build the structure. Workers already on site finish handling carried materials and collected items, then fly back to the warehouse to return their old tool and take the next one. Exchanges never happen remotely. Tools can come from a container or item drops within the single block below the lounge. Creative Crates supply every tool indefinitely without a sample item. When ordinary sources lack a suitable tool, workers use their existing abilities or wait
...
Fire and Nether portal cells do not consume block items. A dedicated worker holding flint and steel or fire charges must deliver the ignition operation. It never carries building blocks at the same time; workers carrying construction materials or collected items refuse player-given ignition items until they unload. Each fire costs 1 flint-and-steel use or 1 fire charge; one connected portal in the same plane also costs only 1 use or 1 charge. Flint-and-steel durability follows vanilla enchantment rules, and workers can replenish exhausted ignition items from the warehouse

Workers wait for fire supports before claiming or delivering ignition. Ordinary fire needs a sturdy top face below or an adjacent flammable block; soul fire needs soul sand or soul soil below. Nether portals wait for a complete valid frame with an interior 2–21 blocks wide and 3–21 blocks high. Existing world blocks and delivered construction projections count as supports and frames; undelivered blueprint cells do not. Waiting consumes no ignition items

Ignition takes effect after the final structure is committed, when supports and frames are checked again. The entire blueprint portal must have been delivered, and portals activate only in the Overworld and Nether. Once ordinary block construction finishes, ignition still lacking valid supports or frames, or using an unsupported dimension, is skipped as incomplete without consuming ignition items or placing invalid fire or portal blocks
