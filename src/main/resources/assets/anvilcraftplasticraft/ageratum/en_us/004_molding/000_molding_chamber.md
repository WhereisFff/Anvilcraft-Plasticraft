...
The chamber uses its model cubes for selection, preserving slanted edges and allowing the crosshair through gaps between cubes in both powered and unpowered appearances. The forming region behind it keeps the interaction bounds defined by the current production stage
...
Without a 3D Printing Component, an ordinary model uses Casting. Hold <ref item="anvilcraftplasticraft:plastic_3d_printing_component"/> and right-click any chamber face to install it exactly one block above, without Shift or opening the GUI. Once installed, every model switches to 3D Printing. The method is fixed when the cycle is locked and cannot switch midway
