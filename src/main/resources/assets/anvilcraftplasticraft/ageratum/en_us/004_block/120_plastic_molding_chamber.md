...
An ordinary Structure Disk keeps AnvilCraft's existing tooltip, Smart Block Placer `5x5x5` check, and rotating block-structure preview. When a Structure Disk contains a molding model, its hover window rotates that model at the same speed and centers it from its exact bounds. Structure shows the model name, Size shows the fully manufactured X, Y, and Z dimensions in blocks, and the final line becomes either a green processing confirmation or a red Plastic Molding Chamber `3x3x3` size warning. Molded plastic items use the same size calculation and show at most four necessary decimal places.

The model management overlay starts closed whenever the GUI opens and shows up to six shared models per page. It renders one button per actual model and no empty model buttons. Search filters filenames, model names, and owners. Long lists scroll vertically, while long names scroll within their row. Selecting a row only selects that entry and writes it to the current Structure Disk; it never changes the chamber model directly. Without a Structure Disk, the selection remains for the current GUI session. Replacing different disk data shows both names and hashes and requires a second click on the same row. Load, Store, the model management toggle, and the four bottom overlay actions activate only when the left mouse button is released over the original button; dragging away before release cancels the action.

Load Disk is the separate action that moves a Structure Disk blueprint into the editor, always unlocked and editable. The model must fit completely inside the `48x48x48 px` workspace, and loading requires an editable machine with no molded clay and no melt in the forming batch. The 8 B staging tank may remain filled and is never changed. Clicking Load for a model over `48 px` on any axis is rejected with the chamber's `3x3x3` maximum-size message; neither the Structure Disk nor the current draft changes. Replacing the current draft with a compatible model still requires confirmation.
...
Imported bounds may exceed `48 px`. Oversized models retain their source dimensions in the shared library and can be written to Structure Disks for tooltip and rotating-preview inspection, but cannot be loaded into the chamber for editing or processing. A model no larger than `48 px` per axis but outside the workspace is never scaled. The GUI first shows a suggested whole-model translation; another normal Refresh Models click confirms the import with that offset. A file change invalidates the pending confirmation.
