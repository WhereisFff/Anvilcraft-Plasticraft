---
navigation:
  title: "Resin Anvil"
  icon: "anvilcraftplasticraft:resin_anvil"
items:
  - anvilcraftplasticraft:resin_anvil
  - anvilcraftplasticraft:hardend_resin_anvil
---

# Resin and Hardened Resin Anvils

<recipe id="anvilcraftplasticraft:resin_anvil"/>

<recipe id="anvilcraftplasticraft:magnetic_resin_anvil"/>

## Resin Anvil

- The resin anvil is a movable entity affected by gravity and buoyancy, and it can attach to all six faces.
- Sneak-use it with any Anvil Hammer to retrieve it directly while preserving magnetism and captured mobs.
- It bounces when it hits blocks or entities from any direction, but right-clicking it never opens an anvil menu.
- A falling block dropped onto it rebounds with diminishing speed, then settles as a block when the collision top aligns with the block grid.
- Every movable plastic product, including hardened resin anvils and universal plastic, also rebounds repeatedly with diminishing speed before settling on its collision top.
- It shares AnvilCraft resin-block mechanics for capturing mobs and spawners, preserving mobs and resentment, and related time-warp recipes.
- Normal mobs must fit the resin block's size limits. Hostile mobs must also have Weakness.
- Fast cooking it in a dry cauldron produces a hardened resin anvil. A magnetic resin anvil remains magnetic after hardening.

## Hardened Resin Anvil

- The hardened resin anvil keeps entity movement, gravity, six-face placement, buoyancy, and collision without the elastic bounce.
- A falling block dropped onto it does not rebound and settles directly when the collision top aligns with the block grid.
- Sneak-use it with any Anvil Hammer to retrieve it directly while preserving magnetism.
- Right-clicking opens the vanilla anvil workflow. Repair and enchantment merging follow vanilla rules, while renaming alone costs no experience and does not increase the prior-work penalty.
- Resin and hardened resin products use fixed material colours and do not support the 16-colour palette.
- Both magnetic variants use an iron base and iron head connector, and participate in magnetic force and acceleration mechanics.
