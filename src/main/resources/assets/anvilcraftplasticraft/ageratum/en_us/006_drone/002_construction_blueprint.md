...
- A construction drone takes materials from the owner's inventory and, within a 128-block discovery range, delivers ordinary blocks, block-entity contents, bucket fluids, exact tank millibuckets, boats, spawn eggs, resin captures and plastic entities as construction projections; delivered cells look like the real block in the world (wood is opaque planks, water is a translucent surface) while the world cell stays air; fences, walls and doors use the finished connected collision after delivery; plastic entities stay display-only during construction and gain hard collision only after commit, and a drone enters the target cell to deliver when every neighbour is still an undelivered block; reach is 1 block and each delivery costs 2,560 FE; after the last delivery it flies out of the blueprint bounds before landing, and flies back to the owner when the owner is outside the site
...
- After demolition, if the job box still has marked drops and a loaded owner collection drone in the same dimension can still accept cargo within 128 blocks of the nearest marked item, the job stays in a bounded debris sweep; it moves on to building once the marks are gone or every collector is full or out of range. After the sweep or when full on site, collection drones fly out of the blueprint bounds before landing so they are not trapped in cells construction is about to occupy. Without a collection drone the drops stay in the world and building starts immediately, with no pause or skip
- After progress exists the anchor can no longer be moved or rotated
- Cancel quietly commits delivered blocks and delivered contents; a drone that already picked up materials flies back to the owner and puts the in-transit items into the inventory, and undelivered cells stay as they are in the world; already placed real fill blocks stay in place and are neither silently deleted nor demolished further; a material or missing-demolition shortage pauses by default, and switching to skip lets the rest continue then finishes incomplete; a permission denial always pauses
- Block-entity container items are taken from the inventory by slot and written into the real inventory on commit, while names, locks and other non-resource machine settings come from the blueprint; a giant anvil or other large multipart uses one core item and delivers the remaining parts with that core; machines stay inert during construction, and after commit vanilla redstone dust is written across the region from the blueprint appearance and power; AnvilCraft redstone wire also writes the blueprint ports into the connection-override table so later network updates neither grow extra arms nor merge parallel lines; power may then follow the live world supply while the ports stay as recorded
- Source liquids take a full bucket and write the fluid on commit, then return the empty bucket; layered cauldrons and block-entity tanks extract the exact millibucket amount and never round to a whole bucket; boats, spawn eggs, captured resin anvils / resin blocks / high-viscosity resin blocks and plastic entities with contents use adapters and real inventory items; a resin block returns 1–3 resin before the next claim, a resin anvil is consumed whole, and the real mob appears only at final commit; players, drops and experience orbs stay skipped; fluids and solid blocks already in the world are still sealed and demolished
