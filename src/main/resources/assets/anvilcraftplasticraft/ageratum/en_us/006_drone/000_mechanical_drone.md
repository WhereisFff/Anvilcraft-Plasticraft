...
- A construction drone discovers work when the nearest executable target is within 128 blocks and has a path, with a 1-block reach; it takes items from the owner's inventory, flies to an approach cell, delivers a construction projection, then returns to the player; after the last delivery it flies out of the blueprint bounds before landing instead of stopping on the site, and flies back to the owner when the owner is outside the site; pickup and returns do not count as instant actions, and each delivery costs 2,560 FE
