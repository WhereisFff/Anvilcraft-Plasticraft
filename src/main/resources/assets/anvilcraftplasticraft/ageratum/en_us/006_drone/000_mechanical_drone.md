...
- A collection drone in free mode rescans its current 16-block radius after every inhale, has a 1-block reach, costs 2,560 FE per item entity, ignores experience orbs, and does not consume magnet durability; when all nine cargo slots are full it lands and stops inhaling; if the owner walks within 16 blocks it unloads into the owner's inventory with leftover stacks staying on the drone; in a demolition or debris job it only claims drops marked for that job, leaving unmarked nearby items for free-mode drones, players, or hoppers; after the sweep finishes or the drone fills up on site it flies out of the blueprint bounds before landing instead of stopping on a cell that construction will occupy; missing collection drones never block construction, so leftover drops stay in the world and building continues
- Delivered construction projections stay as air with no block entity, but they look like the real block in the world (wood is opaque planks, water is a translucent surface) and provide real collision so players can stand on them; fences, walls and doors use the finished connected shape; undelivered holograms can be walked through; empty-collision targets such as redstone dust create no fake collision, and delivered or committed vanilla dust is written across the region from the blueprint appearance and power, while AnvilCraft redstone wire also writes the blueprint ports into the connection-override table so later network updates neither grow extra arms nor merge parallel lines; after commit, power may follow the live world supply while the ports stay as recorded; block-entity contents are taken as real materials, a large multipart consumes only its core item, bucket fluids return an empty bucket, a resin-block capture returns 1–3 resin before the next claim, and a resin anvil is consumed whole; plastic entities stay display-only during construction and gain hard collision only after commit, and a drone enters the target cell to deliver when every neighbour is still an undelivered block
