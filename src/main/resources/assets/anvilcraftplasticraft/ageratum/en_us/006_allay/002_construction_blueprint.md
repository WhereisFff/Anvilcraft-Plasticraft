...
- A construction allay takes materials from the owner's inventory — or any needed item infinitely from a creative owner without consuming the inventory — and, within 128 blocks, delivers normal blocks, block-entity contents, bucket fluids, exact millibucket tanks, boats, spawn eggs, resin captures and plastic entities as construction projections. A delivered cell still looks like the finished block, including faces against remaining hologram cells, while the world cell stays air. Fences, walls and doors use finished adjacency for collision. Plastic entities stay as projections until commit, and the allay may enter the target cell when every neighbor is an undelivered place. Reach is 1 block, and take/place/break/collect happen at most once every 4 gt after it turns to face the target. After the last delivery the allay leaves the blueprint box before hovering, or returns to the owner if they are outside the site
