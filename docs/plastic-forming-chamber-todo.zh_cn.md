...
| 模型管理 | `[324, 171, 16, 16]`，装饰外框 `[320, 167, 24, 26]` | 使用 `model.png` 的四段 `16x16` 贴图，依次为标准、鼠标悬浮、左键按压中、选中，选中且鼠标悬浮时复用第二帧；在按钮内松开左键时切换右叠加层，不等同于磁盘载入 |
...
4. 每个模型行都是两段按钮、贴图 2 段，只有未选中和选中两帧，鼠标悬浮不换帧。列表选择是单选；点击一项会选中该共享模型，并按第 10 节把它写入当前结构磁盘，但不修改成型舱中的模型。没有结构磁盘时仍允许保留列表选中态并提示先放入结构磁盘；结构磁盘已有不同模型时先显示覆盖确认，确认前不改磁盘。视图滚动后选中态不得丢失。
...
11. 支持零厚度、重叠、隐藏状态、负坐标和不从 0 开始的模型。任意受支持模型都允许导入共享蓝图库；三轴完整外接尺寸都不超过 3 个方块（即每轴 48 个 1 px 制造单位）时，允许玩家预览并确认整体平移到 3x3x3 工作空间内，不自动缩放；Blockbench 中从 `(0,0,0)` 开始、大小恰为 `48x48x48` 的模型必须自动以保持 `(0,0,0)` 数值的根变换贴合整个工作空间并直接识别。任一轴超出 3 个方块时保留原始尺寸导入，也允许写入结构磁盘，但不执行平移或制造烘焙；只有点击“加载磁盘”把蓝图送入成型舱继续编辑时才拒绝，并提示最大加工尺寸为 `3x3x3`。导入后的源形状和合格模型的锁定制造预览都要有往返样例验证。
12. 鼠标指向普通方块结构磁盘时，本体“结构/尺寸/智能方块放置器” Tooltip 和旋转方块预览保持不变。结构磁盘存有成型蓝图时，“结构”显示模型名，“尺寸”显示完整加工后可见几何的真实三轴格尺寸，最后一行改为绿色“可以被塑料成型舱加工”或红色“对于塑料成型舱来说太大了（最大 3x3x3）”；`80x80` 悬浮预览改为按真实外接范围居中、以本体同样的每 tick `2°` 速度旋转成型模型。塑料制品 Tooltip 使用同一组数值并统一显示“尺寸”。
13. 把已有成型蓝图的结构磁盘载入成型舱时，只有模型完整位于 `48x48x48 px` 工作空间内、机器处于 `EDITABLE`、成型区域批次熔体为 0 且没有预留粘土时才允许覆盖。8 B 暂存罐可以非空，载入模型不得清空或改写其中的熔体。当前建模区域非空时按第 6 条用标题栏红字二次确认；模型超限、机器已成模、正在加工或成型区域仍有熔体时直接在标题栏说明原因，不能改写结构磁盘、当前草稿或机器资源。
14. 向已有数据的结构磁盘“存入”、从共享模型行向已有数据的结构磁盘写入或用同名文件保存时必须显示覆盖确认，默认生成不冲突文件名。失败的文件写入不能改变结构磁盘原数据；结构磁盘写入和 JSON 原子替换要么都成功，要么保留可恢复的一致状态。
15. 服务器蓝图库位于 `<world>/anvilcraftplasticraft/blueprints/`；客户端只在玩家普通点击“刷新模型”时，从 Shift 打开模型文件夹所对应的安全目录上传白名单扩展名、受限大小和深度的文本。禁止路径穿越、符号链接逃逸、任意服务器路径读取和外部父模型解析；允许的父模型只能来自明确白名单并在导入时展开为自包含蓝图。
16. 手动刷新导入、成功回执后删除未变化的源文件、置顶、复制、Shift 打开模型文件夹和 Shift 二次确认删除的完整交互按第 3.5 节执行。复制和导入产生的新文件归操作者所有；置顶只改变操作者自己的排序；打开文件夹不能泄露远程服务器路径；删除始终由服务端复核权限。
...
**玩家可见结果**：玩家可以随时把当前模型存入结构磁盘，在默认收起的模型管理叠加层中按住 Shift 打开模型文件夹、放入文件并普通点击刷新完成导入，也可搜索、置顶、复制、定位或删除共享模型，再通过结构磁盘把合格模型载入成型舱继续编辑；超限模型仍可导入、写盘并查看真实尺寸和旋转预览。
...
**验收清单**：普通磁盘不能放入，只有结构磁盘可用；无结构磁盘时加载/存入按钮禁用，点击共享模型行只保留选中态并提示放入结构磁盘；放入结构磁盘后点击模型行只写盘、不改成型舱，点击“加载磁盘”才把合格模型送入成型舱；空结构磁盘一次点击存入，非空结构磁盘标题栏提示后再次点击覆盖，未锁定或锁定状态都可存入且失败不损坏原数据；普通方块结构磁盘继续显示本体 Tooltip、智能方块放置器判定和方块预览，成型蓝图结构磁盘改为显示模型名、真实三轴尺寸、成型舱加工判定及同速旋转模型预览；超出 `48x48x48 px` 的模型可导入共享库并写入结构磁盘，但点击“加载磁盘”时提示最大 `3x3x3` 并保持当前草稿和磁盘不变；建模区有 cube 时加载合格模型先在标题栏提示、再次点击清空并导入；载入后不锁定且可编辑；8 B 暂存罐非空时仍可载入且流体不变，只有成型区域有批次熔体时才拒绝覆盖；叠加层每次打开 UI 默认关闭并且只由模型管理按钮切换；列表只绘制实际模型按钮，长名水平滚动、长列表纵向滚动，右侧滑块使用 `overlay_slider.png` 的标准/悬浮两个 `4x12` 半幅且轨道按规定调整，列表不足七项时保持标准帧且没有禁用灰层；没有选中模型时仍能按住 Shift 打开目录，放入白名单文件后不会后台自动导入，普通点击刷新才扫描；服务端导入成功后删除未变化的源文件，失败或源文件已变化时保留；需要整体平移时再次普通点击刷新确认；Blockbench `(0,0,0)` 映射到成型舱 `(0,0,0)` 且 `48x48x48` 模型可识别；置顶不改其他玩家排序；存入/导入/复制的 owner 正确；远程服务器只打开本地只读导出副本且该副本不回传；删除与重名复核权限和修订号；结构磁盘在 JSON 丢失后仍可载入；不支持的模型元素列出具体错误而不静默丢形状。

**本项不包含**：生产锁定、塑料加工和托盘功能。

### TODO-03 供电、粘土模具、熔体缓冲和自动化状态机

**玩家可见结果**：玩家可以在任何机器阶段从顶部或底部管道或资源槽向 8 B 暂存罐输入塑料熔体，也可用资源槽中的电容器给成型舱供电并投入粘土球，手动或用红石锁定模型；区域无实体时，粘土逐层动画与完整碰撞同时开始，模具完成后机器再以固定 `250 mB/gt` 把暂存熔体泵入成型区域，使单批累计量可以超过 8 B。
