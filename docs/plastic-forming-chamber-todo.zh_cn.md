...
> 状态：设计稿，尚未进入 TODO 实现阶段，也不是最终实现与验收基准
>
> 适用项目：Plasticraft / AnvilCraft 附属模组，版本为 NeoForge 1.21.1
>
> 后续流程：下一阶段先按第 0 章完成模型和贴图；资源齐全后再修订并发布最终实现版文档。只有最终实现版和届时已经落库的资源文件共同构成 TODO 的实现与验收基准。
>
...
| 资源           | 需要制作的文件   | 内容                                                      |
|--------------|-----------|---------------------------------------------------------|
| 塑料成型舱未锁定模型   | 方块模型 JSON | 成型舱未锁定时使用的完整方块模型                                        |
| 塑料成型舱锁定模型    | 方块模型 JSON | 成型舱锁定后使用的完整方块模型                                         |
| 塑料成型区边框      | 方块模型 JSON | 成型舱后方的3x3x3区域的边框模型                                      |
| 塑料成型舱物品模型    | 物品模型 JSON | 背包、手持和掉落时使用的模型                                          |
| 成型舱 GUI 主背景  | PNG 贴图    | 成型舱主界面的完整背景和各功能区域底图                                     |
| 类型选择叠加层      | PNG 贴图    | 点击“指定类型”后从主界面左侧展开的叠加层背景                                 |
| JSON 管理叠加层   | PNG 贴图    | 点击“JSON 管理”后展开的叠加层背景                                    |
| 成型舱 GUI 控件图集 | PNG 图集    | 全部建模操作按钮、类型指定按钮与图标、机器控制按钮、磁盘与 JSON 管理按钮、输入框、滚动条、槽位和资源条； |
| 塑料基础贴图       | `plastic_base.png` | 程序化塑料贴图使用的 16x16 灰度基准贴图                              |
...
1. 建模部分是嵌在成型舱 GUI 中的高精度三维显示与交互区域：内部拥有场景树、相机、选择、变换、撤销栈和渲染循环，但屏幕上可见的建模按钮、属性栏、元素列表和相机按钮全部属于成型舱 GUI，不在嵌入区域内再做一套界面。嵌入区域负责不受普通 Minecraft `Screen` 像素级显示能力限制的模型显示、命中、操纵轴和直接拖拽，也不弹出脱离游戏的外部窗口。
...
4. 若采用嵌入式网页运行时，只加载模组内打包的本地 HTML/JavaScript 和三维显示所需库，不访问互联网、不执行外部脚本，也不向页面暴露任意系统文件权限。成型舱 GUI 继续绘制全部按钮和面板，网页区域不能复制一套工具栏。Blockbench 的代码、显示方式或快捷键若被直接复用，必须先完成许可证和再分发检查；否则自行实现相同工作流。
5. 成型舱 GUI 宿主建模工具按钮、相机按钮、数值属性、元素列表、机器槽位、能量、生产模式、磁盘和会话生命周期；嵌入区域只负责三维显示、命中和直接模型交互。两者通过有 schema、字段和整体大小限制、会话 ID 与修订号的窄桥接通信：打开会话、载入快照、提交编辑命令、撤销/重做、导入/导出、请求分析、锁定和关闭会话。
...
下图是当前已经确定的 GUI 设计稿。红框为常驻主界面；左侧蓝框是点击“指定类型”后展开的类型选择叠加层，右侧蓝框是点击“JSON 管理”后展开的模型管理叠加层。草图中的文字只说明功能和大致位置，不是最终美术稿，也不是可直接照图实现的像素坐标规格；资源制作与最终版修订期间可以调整具体间距和比例，但不得遗漏本节已经确定的控件、状态或三部分界面关系。

![塑料成型舱 GUI 设计草图](assets/plastic-forming-chamber/gui-layout-draft.png)

当前设计稿阶段不提供元素坐标清单。所有模型、贴图及其他 GUI 资源完成后，最终实现版必须在本节定义统一的 GUI 像素坐标原点，并逐项列出常驻主界面、建模程序宿主区域、玩家物品栏、类型选择叠加层和 JSON 管理叠加层中每一个元素的像素坐标位置。TODO 实现只能使用该最终坐标规格，不能测量本草图或凭视觉比例反推坐标。

| 大致位置    | 区域        | 必须包含的内容                                                                |
|---------|-----------|------------------------------------------------------------------------|
| 顶部整行    | 标题栏       | “塑料成型舱”标题、关闭入口，以及不遮挡建模的宿主窗口区域                                          |
| 左侧竖栏    | 建模工具与类型入口 | 草图指定的移动、缩放尺寸、旋转、镜像，以及新建 cube/平面、选择等必要扩展工具；下部放置“指定类型”按钮；当前工具有明确选中态和悬浮提示 |
| 主界面左侧外扩 | 类型选择叠加层   | 搜索框、普通制品/箱子/储罐类型列表、图标、名称、选中态和滚动条；                                      |
...
| 右栏属性区内  | 制造分析      | 当前指定类型、验证结果、容量、实体积、粘土总需求、当前可成型体积和部分加工降级警告                              |
...
8. 主界面和左右两侧叠加层必须适配小窗口、高 GUI 缩放和超宽屏。属性栏与玩家物品栏可按预定断点重排，但不得让按钮、文字、槽位、方向标识或建模视口互相覆盖。建模视口必须保留可实际操作的最小尺寸，不能被物品栏压成只能预览的小窗。

### 3.4 类型选择叠加层

1. “指定类型”按钮固定在主界面左侧建模工具栏下部。点击后从主界面左侧展开类型选择叠加层，再点击同一按钮关闭；每次新打开成型舱 UI 时叠加层默认关闭，当前已指定类型仍保存在成型舱中。
2. 顶部搜索框按本地化名称过滤类型。下方列表来自类型注册表，每项显示图标、名称和选中态；当前版本只列出普通制品、箱子和储罐，普通制品为默认项，托盘在功能补完前不显示。项目超过可视范围时使用右侧纵向滚动条。
3. 选择类型只更新当前模型的指定类型并立即请求服务器重新验证，不会锁定或加工模型。普通制品始终有效；箱子或储罐验证失败时保留玩家选择和编辑状态，在制造分析区显示“这个形状好像不能做这件事”及具体原因，并在修正形状前禁止锁定。
4. 左侧类型叠加层和右侧 JSON 管理叠加层分别由各自按钮控制，打开或关闭任一侧都不会修改模型、选择、撤销历史或另一侧的开关状态。两侧叠加层的展开状态都不持久化。

### 3.5 JSON 管理叠加层

1. 草图中的右侧蓝框是从主界面右侧展开的 JSON 管理叠加层。每次新打开成型舱 UI 时它都默认关闭；点击“JSON 管理”展开，再点击同一按钮关闭，不记忆上次开关状态。
...
7. 设落砧加工快照中成型区域已有 `F` mB 熔体。通过 250 mB 最低门槛后，实际有体积部分取 `fillOrder` 的前 `min(V, 4F)` 个单元；同一 mB 对应的 4 个单元仍严格按既定顺序依次选择。
8. 当 `V > 1000` 且 `F < ceil(V / 4)` 时得到部分结果。最大模型可在外部管道持续补充暂存罐时累计泵入 `27648 mB` 并完整成型。
...
10. 纯平面模型在成型区域达到 250 mB 后可单独加工；这 250 mB 同时提供成品的材料类型和颜色。
11. 普通模型中的平面只在至少一个相邻模型体积单元已经成型时进入部分结果；没有任何体积单元的纯平面模型在满足 250 mB 后包含全部平面。
...
15. 暂存罐中有熔体不妨碍编辑、重置模型、手动解锁或从磁盘载入模型。只有成型区域内已经存在批次熔体时，才拒绝解锁、重置或覆盖模型，并提示先从底面抽空成型区域；这些操作不得自动删除任何一处熔体。
...
2. 制品数据至少保存：格式版本、实际成型形状、模型哈希、材料、颜色、朝向面、面内四分之一转角、功能类型、容量、名称、箱子物品或储罐流体内容。
...
6. 物品堆叠必须比较完整制品数据。带箱子物品、储罐流体或其他可变内容的制品不可堆叠；无内容制品也只有在实际形状、材料、颜色、朝向、功能和格式版本完全一致时才能堆叠。
...
2. GUI 包含本体磁盘专用格、格子上方的“存入”、格子下方的“载入”，以及独立的“JSON 管理”按钮。无磁盘时存入/载入禁用；放入空磁盘后灰色“存入”按钮进入可点击显示，未锁定时点击仍提示“需要先锁定”。“JSON 管理”不依赖磁盘，只负责切换第 3.5 节叠加层。
...
15. 上传、置顶、复制、打开模型文件夹和 Shift 二次确认删除的完整交互按第 3.5 节执行。复制和上传产生的新文件归操作者所有；置顶只改变操作者自己的排序；打开文件夹不能泄露远程服务器路径；删除始终由服务端复核权限。
...
5. 模型、互斥模式、红石上次电平、待锁定请求、动画进度、槽内/已成模粘土、8 B 暂存熔体、成型区域批次熔体、内部 FE、磁盘和等待原因都要分别持久化。加工结果不进入机器持久化数据，粘土动画结束后也没有额外等待状态。类型选择和 JSON 管理两个叠加层的开关都不持久化，每次打开 UI 都从关闭开始。
...
> **实现启动条件**：TODO-00 是可提前独立开发的基础任务，不受成型舱其他美工资源完成情况限制，经用户确认本 TODO 后即可单独实施。TODO-01 至 TODO-06 仍需等待第 0 章中与成型舱有关的美工资源完成，并且本文被修订为“最终实现版”，不能把当前设计稿直接当作这些任务的编码依据。
...
4. 依赖关系为：TODO-00 与成型舱基础任务相互独立，可以最先完成或与 TODO-01 至 TODO-03 并行；TODO-01 是成型舱其余任务的基础；TODO-03 可在 TODO-01 后实现；TODO-02 依赖 TODO-03 提供的真实锁定状态；TODO-04 同时依赖 TODO-00 和 TODO-01；TODO-05 依赖 TODO-03 和 TODO-04；TODO-06 依赖 TODO-04 和 TODO-05。TODO-02 不是生产链的硬依赖，但最终发布前必须完成。
5. 托盘功能不属于下面任何 TODO。未来必须在实际功能定义后新写规格和 TODO，不得在完成本期任务时顺手实现猜测版托盘。

### TODO-00 通用塑料块与任意形状塑料贴图生成器

**玩家可见结果**：以流体方块形式放在世界中的一格通用塑料熔体，无论由哪一种世界机制使其冷却、凝固、硬化或转化，都不再掉落 16 个塑料粒，而是一律在原位置变成一个长宽各 `16 px`、高 `14 px` 的同色通用塑料块。该方块是没有额外功能的普通塑料制品，拥有现有塑料实体的全部物理与交互；它的贴图由通用生成器根据 `plastic_base.png` 生成。塑料粒只保留在本 TODO 明确列出的配方与输出口入水锅流程中，珠宝商会用 2 个绿宝石收购 16 个塑料粒。

**前置依赖**：无。输入资源 `src/main/resources/assets/anvilcraftplasticraft/textures/palette/plastic_base.png` 已作为 16x16 灰度基准提供；本 TODO 可以在成型舱和自由制品尚未实现时独立完成。

**规格范围**：程序化塑料贴图公共接口与客户端资源生命周期；通用塑料块/物品/实体；`UniversalPlasticMeltFluidBlock` 所有世界内冷却、凝固、硬化或转化入口的统一结果；现有塑料粒配方和 `CatalyticPressProcess` 输出口入水锅分支；AnvilCraft 珠宝商交易；相关数据生成、Tooltip、Ageratum 手册和测试。

**必须交付**：

1. 建立与具体方块、实体和成型舱解耦的版本化贴图生成接口。输入是稳定形状哈希、外露表面及其局部坐标/法线/双面标记、基础贴图和色板标识；输出是确定性的自动 UV 图集及表面到 UV 的映射。接口必须能直接接收未来任意塑料制品的凹形、断开、旋转及无厚度双面表面，不能以 `UniversalPlasticBlock` 类型判断或固定六个长方体面作为公共 API。
2. 通过资源管理器读取 `plastic_base.png`，按稳定纹素密度把它作为各表面的塑料纹理基准，并把边缘、凹槽和邻近遮蔽等形状修饰量化回灰度级。生成器、灰度数据结构和映射代码统一支持最多 16 个有序灰度级；当前基础贴图只有 6 个灰度值，必须原样识别这 6 级，但实现不得写死“只能 6 级”、当前 RGB 值或现有资源的级数。1 至 16 级的基础资源均可使用，超过 16 级、非灰度或非法图片必须给出明确资源错误并使用安全占位，不能使客户端崩溃。
3. 灰度结果接入现有塑料色板，至少完整支持 `PlasticMeltColor` 的 16 种 `DyeColor`。现有 `universal_plastic_palette.png` 只有 8 个有序灰度级，本 TODO 允许并要求同步修改该色板资源及其读取、索引和映射代码，使每种颜色都提供并可寻址最多 16 个灰度级；不得通过重复相邻色阶伪装成 16 级，也不得保留任何 8 级常量或位宽假设。相同形状、生成器版本、基础资源哈希、色板资源哈希和颜色必须得到逐字节一致的结果。CPU 生成可异步执行，GPU 上传只在渲染线程提交；缓存按上述键复用，并在资源重载、退出世界和显存回收时释放。服务器只保存形状、版本和颜色等权威数据，不同步生成后的 PNG 或显存对象。
4. 注册单一“通用塑料块”方块、物品和塑料实体。当前仓库中用户所称的 `AbstractPlasticBlock` 对应实际基类 `AbstractPlasticEntityBlock<E>`；新方块继承该类，新实体继承 `AbstractPlasticEntity`。方块局部形状固定为 `0,0,0 -> 16,14,16`，方块轮廓、碰撞、实体尺寸、射线选取和渲染必须一致；物品、临时方块态、实体、粘合后的方块态和锤回物品都保留熔体颜色。它没有菜单、库存、流体能力、容量或其他专用功能。
5. 凡是以 `UniversalPlasticMeltFluidBlock` 流体方块形式放置在世界中的通用塑料熔体，所有会让该流体方块冷却、凝固、硬化或转化的世界内触发入口都必须汇入同一结算路径，并在原位置替换为一个通用塑料块的方块状态。雨、相邻水和 `PLASTIC_MELT_COOLANTS` 标签只是当前触发示例，不是限定哪些触发会生成塑料块的白名单；实现时必须审计所有现有世界流体方块转化入口，不能遗留任何产出塑料粒或其他结果的分支，今后新增同类触发也必须复用这条统一路径。结果继承熔体的 `PlasticMeltColor`，原熔体方块只结算一次，不得同时生成塑料粒；该通用塑料块此后仍可按其继承的塑料物理机制正常在方块态与实体态之间转换。
6. 塑料粒只保留在这里明确列出的配方或容器处理流程中，不得全局替换：大型炼药锅的 `fluid_mixing/universal_plastic_melt_with_water` 和 `fluid_mixing/universal_plastic_melt_with_powder_snow` 仍按每 `1000 mB` 熔体产出 16 个同色塑料粒；既有 `solid_liquid/cool_universal_plastic_melt` 也保持原产物。另在催化压盖的输出口路径中加入原子分支：输出口面对装满水的炼药锅时，消耗源容器中的 `1000 mB` 通用塑料熔体和目标锅中的一满锅水，在目标锅位置产生 16 个继承熔体颜色的塑料粒，不产生通用塑料块；熔体不足、目标不是满水锅或事务预检失败时不得吞掉任一侧资源。这些都是配方或容器处理，不属于 `UniversalPlasticMeltFluidBlock` 流体方块的世界转化，不能据此为世界中的熔体保留任何塑料粒产出分支。
7. 通过 NeoForge `VillagerTradesEvent` 向 AnvilCraft `ModVillagers.JEWELER` 增加一级收购交易：珠宝商收取 16 个任意颜色的通用塑料粒并支付 2 个绿宝石。交易参数沿用本体一级收购交易的 `16` 次最大使用、`2` 点村民经验和 `0.05F` 价格倍率，不能覆盖或复制本体已有交易。
8. 为通用塑料块补齐注册、语言、物品组、标签、掉落/回收、颜色数据、Tooltip 和中英文 Ageratum 说明；手册明确区分“世界中的熔体流体方块以任何方式转化都得到通用塑料块”和“指定配方或输出口水锅处理得到塑料粒”。修改注册或数据生成源后运行 `runData`，并为生成器、世界流体方块统一转化事务、保留的塑料粒路径、珠宝商交易和公共塑料物理建立自动测试。

**验收清单**：`plastic_base.png` 当前的 6 个灰度级全部保留，包含第 7 至第 16 级的人工测试资源均可被正确读取和映射，第 17 级被明确拒绝；更新后的 `universal_plastic_palette.png` 为每种颜色提供 16 个实际可区分且可寻址的有序色阶，生成结果能够实际覆盖全部 16 级；同一输入重复生成的图集和 UV 完全一致，16 种颜色均来自同一通用路径；16x14x16 通用塑料块的方块、物品、实体和粘合状态外观与碰撞不漂移，所有继承物理都有回归测试；逐项覆盖所有现有 `UniversalPlasticMeltFluidBlock` 世界转化入口，至少包括雨、相邻水和冷却标签，确认每种触发都只在原位置生成一个同色通用塑料块且绝不产生塑料粒；两份大型炼药锅流体混合配方、既有固液冷却配方和输出口满水锅路径都各自产出 16 个同色塑料粒；输出口失败回滚不吞水或熔体；一级珠宝商能够以 2 个绿宝石收购任意颜色的 16 个塑料粒；资源重载与退出世界后无纹理或显存缓存泄漏。完成前运行 `./gradlew runData`、`./gradlew build` 和 `./gradlew runGameTestServer`，游戏内最终纹理观感由用户手动验收。

**本项不包含**：塑料成型舱、嵌入式建模器、动态自由制品或其生产链。生成器的公共接口必须已经能够接收这些未来制品的任意表面数据，但本 TODO 只用通用塑料块和合成测试形状完成真实接入与验收。
...
4. 按第 3.3 节完成主界面布局、玩家物品栏、右侧属性/元素栏、资源与磁盘控件的宿主位置、Shift 二次点击重置、左侧“指定类型”按钮和类型叠加层宿主，以及可打开/关闭但尚未连接共享文件数据的 JSON 叠加层容器。类型注册与验证归 TODO-06，具体磁盘/文件功能归 TODO-02。
...
**必须交付**：复用 `DiskItem`/`DISK_DATA`；磁盘专用槽、上方“存入”/下方“载入”按钮和二次载入流程；磁盘内嵌蓝图与哈希；服务端共享 JSON 目录、owner/管理员权限与原子写入；草图中右侧蓝色叠加层的完整列表和滚动交互；上传、搜索、置顶、复制、打开文件夹和 Shift 二次点击删除；Minecraft Java JSON 与受支持 Blockbench `.bbmodel` 导入；路径穿越、符号链接逃逸、超大/过深文件和非法元素防护。
...
**必须交付**：`256 kW` AnvilCraft 用电接入与等同一个超级电容器的内部 FE 储能；只收粘土球的单堆叠过滤槽与物品能力；粘土需求计算、边收料边成模、暂停/恢复/解锁退还/破坏掉落；首版 `12 gt` 填充动画、动画启动前一次实体占用检查、阻塞 GUI 提示、启动事务内立即启用的 3x3x3 完整碰撞和完整粘土纹理；底面固定的双向流体能力、任何阶段均可外部输入的独立 8 B 暂存罐、锁定且模具完成后固定 `2 B/gt` 向按 `1 mB = 4` 立方像素计算的独立 `M(V)` 成型区域泵送、反向抽出、流体身份/颜色一致性和成型区域最少 250 mB；三种互斥模式、红石上升沿锁存、持续高电平去重、区域/缺料/供电阻塞恢复；8 B 暂存与成型区域熔体的独立持久化及破坏时舍弃暂存熔体；为 TODO-05 提供只结算成型区域熔体的原子加工快照和成功/失败状态转移。

**验收清单**：单槽从 64 球用完后可由溜槽继续补到理论最多 108 球，不存在隐藏无上限槽；实体阻塞时动画不开始且 GUI 明确提示，移走后只检查一次并在动画第一帧启用完整碰撞，动画末层不做第二次占用检查；动画因缺料/失电暂停时碰撞保留；8 B 暂存罐在空模型、编辑、断电、未成模、加工和等待状态均可被外部输入；泵送只有锁定且模具完成并有电时发生，实测速率每 gt 不超过且能达到 2000 mB；暂存罐和成型区域可同时达到 `8000 mB + M(V)`，最大模型区域可累计 `27648 mB`，两处合计 `35648 mB`；暂存熔体不妨碍解锁或覆盖，成型区域有熔体才拒绝；破坏机器不掉落或保留 8 B 暂存熔体；默认红石模式只响应一次上升沿；单次模式忽略红石；区块重载不重复取粘土、泵送或丢熔体。

**本项不包含**：巨型铁砧产出成品、成品渲染/碰撞和功能箱罐；加工成功后的粘土实际掉落事务在 TODO-05 与制品产出一起接入。

### TODO-04 24 朝向动态制品、自动碰撞和物理机制

**玩家可见结果**：任意已制造形状都能成为可持有、放置、回收和参与现有 Plasticraft 物理的动态塑料制品，放置时保留 24 种姿态、真实形状外观和碰撞。

**前置依赖**：TODO-00 和 TODO-01。必须直接复用 TODO-00 已验收的通用贴图生成接口，不得在本 TODO 内另做一套自由制品专用生成器。

**规格范围**：第 7 节，并复用第 4.1 节的派生形状。

**必须交付**：

1. 实现单一动态制品物品 ID、数据组件、单一动态实体 ID、物品/实体渲染、放置/锤回/存档/同步/跨维度往返和完整数据堆叠规则。
2. 把实际完整或部分成型的 `surfaceMesh`、模型哈希、双面标记、材料和颜色转换为 TODO-00 的公共生成器输入；物品、放置预览、动态实体和粘合方块态必须共用同一生成结果，不能手写 UV、上传服务端 PNG 或按具体功能类型分叉渲染。
...
**验收清单**：完整、部分、凹形、断开和纯平面模型均通过 TODO-00 的同一接口正确显示且不丢面；物品、预览、实体和粘合状态没有纹理方向或锚点跳变；24 个姿态逐一往返，没有丢失面内旋转或锚点偏移；平面可选取/锤回但无物理碰撞；凹形与断开模型碰撞不变成实心外框；全部现有塑料物理机制在大模型和全部姿态下有自动回归测试。

**本项不包含**：程序化贴图生成算法、巨型铁砧触发生产和箱子/储罐容器能力；生成算法及缓存生命周期只归 TODO-00。
...
**必须交付**：由成型舱锚点推导的顶层 3x3 工作台/空间超压器两种结构；与 AnvilCraft 巨型铁砧落地事件的动态处理桥；服务器原子预检和提交；按 `1 mB = 4` 立方像素及 `(-Y -> +Y, -X -> +X, -Z -> +Z)` 的部分成型；材料/颜色/实际形状快照；在成型区域原地产生物品掉落或动态实体，机器内部不保存加工结果；水平与斜面元素的部分结果；只消耗成型区域熔体并保留 8 B 暂存余量；三种生产模式回环；固化与粘土块破坏粒子；清零已成模计数并在同一区域按加工前精确数量生成可回收粘土球掉落物；防止与本体静态多方块配方重复命中；成型舱、两种顶层结构、最低熔体和三种模式的 Ageratum/游戏内手册与 JEI 说明。

**验收清单**：中心有空间超压器只在 3x3x3 成型区域产出物品掉落，完整 3x3 工作台只在同一区域产出实体；工作台、空间超压器、成型舱和区域部件不被消耗；熔体不满时按顺序只产出当前部分形状，250 mB 对应前 1000 个立方像素，下一 1 mB 再对应后续最多 4 个；冲击不会把 8 B 暂存熔体瞬间并入加工，成功后只清空成型区域且暂存量逐 mB 不变；任意预检失败不吞熔体/粘土或产生半件成品；输入 1、64、108 球的测试必须分别在成型区域掉回 1、64、108 球，不因堆叠合并、与满体积制品相交或区块重载而重复/减少；物品制品、实体制品或回收粘土未移走时，连续模式显示区域阻塞且不开始下一副模具，全部移走并把粘土经物流送回后可继续；红石和单次模式加工后回到规定状态；四个成型舱朝向的顶层结构均准确命中。
...
**规格范围**：第 3.4 节、第 8.1 至 8.4 节和第 9 节；第 8.5 节的托盘预留设计明确不在范围内。

**必须交付**：可扩展类型注册表、普通/箱子/储罐稳定 ID、左侧“指定类型”按钮与可搜索滚动的类型叠加层、当前选择与失败提示；带 padding 的六邻接外壳/内腔洪水扫描、无厚度阻隔面、多腔求和、内部装饰拒绝和 `C/(C+S) >= 0.5`；箱子每 64 内腔单元一格、储罐每 171 内腔单元 1 B 的容量派生；部分成型重新验证和降级普通制品；无菜单任意容量物品能力；与 AnvilCraft 大型流体储罐同语义的无菜单共享容量多流体能力；内容随物品/实体往返保存与安全破坏排出；Jade/铁砧锤浮窗分页/汇总；任意重力向量下的内腔网格、等体积半空间裁切、多流体分层、透明排序、重力插值和拓扑缓存。

**验收清单**：类型叠加层默认关闭且只由“指定类型”按钮切换，搜索、滚动和选中态正确，选择无效箱子/储罐时保留编辑并禁止锁定；平面可密封但无碰撞；多腔求和且内部装饰导致验证失败；箱子 `1728 -> 27`、`63 -> 失败`、`64 -> 1`；储罐 `2744 -> 16 B`、`170 -> 失败`、`171 -> 1 B`；容量不设额外上限；大容量浮窗不爆包；多种流体的 ID、组件、数量和顺序完整往返；0°/30°/45°/90° 和连续变化的重力下液体体积不变；凹形、细管、多腔和平面密封模型不穿模、不留非预期空洞、流体层不重叠；托盘不出现可用选项或猜测行为。
