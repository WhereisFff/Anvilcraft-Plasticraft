...
> 状态：TODO-01 至 TODO-06 及支架托盘能力已实现并进入验收；统一塑料实体碰撞重构详见第 13 节；未完成美术资源继续使用占位贴图和占位模型
>
...
| 模型管理叠加层   | 已完成      | `assets/anvilcraftplasticraft/textures/gui/background/plastic_molding_chamber_right.png`，`73x131` | 点击“模型管理”后从主界面右侧展开 |
| 成型舱 GUI 控件图集 | 部分完成     | PNG 图集 | 移动、调整大小、旋转、枢轴、镜像、Cube、新建 Cube、撤销、复制、剪切、粘贴、磁盘载入/存入、模型管理入口及模型管理底部操作按钮已使用正式贴图；锁定等其余按钮继续使用占位帧并保持单帧尺寸和帧数 |
| 塑料铁砧 GUI 着色底图 | 占位实现 | `assets/anvilcraftplasticraft/textures/gui/background/{universal,engineering,heat_resistant,clear}_plastic_anvil_base.png`，`256x256`（绘制 `176x166` 区域） | 四种塑料材料各使用独立底图，按材料显示规则着色 |
| 塑料铁砧 GUI 覆盖图 | 占位实现 | `assets/anvilcraftplasticraft/textures/gui/background/{universal,engineering,heat_resistant,clear}_plastic_anvil_overlay.png`，`256x256`（绘制 `176x166` 区域） | 四种塑料材料各使用独立覆盖图；透明塑料额外叠加低强度的对应色板颜色，覆盖图仍保持不透明 |
...
4. “普通制品/无功能”是默认类型且不受功能形状算法限制；当前实现箱子、储罐、铁砧和支架四种功能类型，支架成品具有托盘能力。模型仍至少要包含一个有体积 cube 或一个零厚度 cube，完全空白的模型不能锁定或加工。
...
| 模具完成/灌入熔体   | 27 格外观均为填满的粘土块纹理，内部熔体不穿透显示       | 完整 3x3x3 碰撞                      | 随时允许输入和抽出           | 有效供电且模型保持锁定时，以固定 `250 mB/gt` 泵送至 `M(V)` |
...
10. 自动检查至少应禁止建模器渲染包出现 `org.lwjgl.opengl` 与 `GlStateManager` 引用，并覆盖纯 CPU 的投影/反投影、六种正交相机、透视相机、视口边界、GUI 缩放换算、有厚度/零厚度 cube 与操纵轴命中、旋转 cube 局部调整大小轴的绘制/命中/拖拽一致性、cube 在正尺寸、`0 px` 和负尺寸间双向跨越缩放及拖拽命令合并。Windows/Linux、窗口化/全屏、高 DPI、全部 GUI 缩放、资源重载及反复打开关闭后的视觉、焦点和 GPU 资源释放仍由游戏内原型和人工验收确认。
...
2. 模型源数据固定使用世界轴：`+X` 指向东、对应 W/E 红轴，`+Y` 指向上、对应 U/D 绿轴，`+Z` 指向南、对应 N/S 蓝轴。三轴边界均为 `0..48`，有体积单元索引为 `0..47`，零厚度 cube 可位于边界或内部格线上。这些数值是工作空间的内部制造单位，不把整个程序定义为像素画布。成型舱朝向只决定整个工作区相对控制器的世界落点以及控制器预览的位置和朝向，不能改变任一模型端点、尺寸符号、旋转轴或内部计量顺序；单选 cube 的调整大小操纵器只按 cube 自身及父分组姿态显示局部轴，不把成型舱朝向写入模型数据。
...
5. 视口必须持续显示固定的世界方向。右上方向标记以中心为原点绘制随相机转动的红绿蓝三条线：红线永远连接 W/E 并表示世界 X 轴，蓝线永远连接 N/S 并表示世界 Z 轴，绿线连接 U/D 并表示世界 Y 轴；线端与字母之间保留清晰间距，不能相连或重叠。模型、参考线、数值、命中射线及移动、枢轴、旋转和镜像操纵器直接使用同一世界 X/Y/Z；单选 cube 的调整大小操纵器使用该 cube 的局部 X/Y/Z，多选时回退到世界轴。所有方向都不得根据成型舱朝向交换轴、反转端点或补偿绕序。默认视角始终使世界北方位于左下；改变成型舱朝向不改变 GUI 中的模型观察方向，只改变控制器预览及世界工作区相对控制器的落点。
...
7. 至少支持：元素与分组列表、新建 cube、选择/多选、移动、尺寸修改、枢轴点、三轴旋转、三轴镜像、复制、删除、隐藏、锁定元素、重命名、分组、居中、对齐、撤销/重做、剪切/复制/粘贴和精确数值输入。cube 的任一轴尺寸保留 `to - from` 的符号，允许为负值并以 `from` 为起点向对应负轴延伸，不得自动交换端点或取绝对值；任一轴也允许缩放到 `0 px` 成为零厚度 cube，并可从 `0 px` 沿正轴或负轴拉伸回有体积 cube。不得保存第二种平面元素类型，同时不得允许两个轴同时为 0 而退化成线或点。操纵轴与右侧数值属性双向同步并进入同一撤销历史；历史最多保留最近 10 个已经提交的语义命令，一次连续拖拽只占一步。操纵器轴长固定为 28 个视口逻辑像素，不随相机缩放：移动与枢轴使用正向三轴箭头，缩放使用正负六向轴和方形端柄；单选一个 cube 时六向缩放轴必须经过该元素及父分组的完整线性变换，绘制、悬停命中和拖拽距离换算共用同一基底，拖动后仍修改对应的局部有符号尺寸；多选因各元素可能具有不同姿态而使用世界轴。旋转在当前枢轴显示半径 2.8 px 的球心、半径 26 px 且只绘制朝向相机部分的三色环，以及半径 27.5 px、完整面向屏幕并紧贴三色环的白色外环，镜像使用正向三轴和菱形镜面柄；轴杆宽 0.75 px、旋转三色环宽 0.9 px、白色外环宽 1.1 px、命中半径 4.5 px，末端尺寸不随轴杆变细。鼠标指向源元素或任一操纵轴时，对应几何覆盖半透明白色悬停层，视觉与 CPU 命中代理必须使用同一屏幕尺度。
8. 不出现 UV、贴图、绘画和手工颜色编辑功能。材料/颜色显示只是基于目前舱内存储的熔体的加工结果预览，不写入蓝图替代最终熔体数据。
9. 每次有效编辑都立即更新世界内同坐标、无碰撞的空间投影，并重算实体积、最低熔体消耗、粘土需求、所选功能类型、验证结果和容量。TODO-01 的 cube 在 GUI 中使用白色不透明材质基色，世界投影使用与施工蓝图投影相同的整体半透明（先写深度再上色，重叠 Cube 不得层层叠深，负形空洞不得被内部面填满），并在 GUI 与世界投影中按精确源表面的实际法线计算连续漫反射明暗；世界投影还按对应世界位置采样天空光和方块光，不得使用全亮渲染。视口与世界投影均显示精确源元素和正常连续斜面，不以体素边界拟合可见外轮廓。两者和最终实体共用第 2.1 节的锚点，模型 `+X` 始终指向世界东、`+Z` 始终指向世界南；整个工作区只按机器背面方向平移到 27 格区域，近边与成型舱背面贴合，不允许旋转、镜像、半格或另一套“渲染上看起来对齐”的隐式偏移。世界参考线和模型统一向上抬升 `0.05 px`，避免与地面深度冲突。建模页的网格、外轮廓、元素边线、操纵器圆环和方向轴均使用固定屏幕像素宽度的实色抗锯齿矢量几何，缩放时不得变成半透明虚线、断线或明显阶梯；相交线和折角必须连续、尖锐且没有线段间的层叠遮挡感，GUI 离屏目标使用高分辨率采样并线性缩小。
...
2. 令主背景可见范围左上角为屏幕坐标 `G = (leftPos, topPos)`，本文所有主界面坐标均相对 `G`。矩形统一写作 `[x, y, w, h]`，采用左闭右开范围 `[x, x+w) x [y, y+h)`；例如移动工具 `[9, 19, 16, 16]` 覆盖本地像素 `x=9..24`、`y=19..34`。独立按钮的矩形只取凹凸边框内的纯色区域，同时作为控件贴图单帧尺寸和鼠标命中范围；其外侧深色左/上线、浅色右/下线及阴影属于背景装饰，不计入按钮。元素列表、类型列表和模型管理列表这类在整片凸起区域中划分行或格的控件是例外：保留完整框起区域并按表中行、格边界划分，不按纯色区域内缩。槽位矩形仍取完整物品格；表中另写“内容区”时，物品、液面或文字只画在内容区内。
3. GUI 缩放、窗口 DPI 和为防止叠加层出屏而做的整体平移只能改变从本地坐标到屏幕坐标的统一变换，不能改变任何本地矩形、元素间距或视口纵横比。缩放后每个逻辑像素必须对应整数个 framebuffer 像素，布局原点必须取整；绘制、鼠标命中、滚轮和离屏视口反投影必须复用同一变换。
4. 左叠加层以主界面本地坐标 `(-72, 72)` 绘制，右叠加层以 `(348, 72)` 绘制。两张叠加层都是 `73x131`；新增的 `1 px` 位于贴图底部，叠加层及其内部元素保持原坐标。连接尖角分别与“指定类型”和“模型管理”按钮中心对齐。关闭叠加层时完全不绘制也不接受命中。
...
| Shift 两组三段按钮、贴图 6 段 | 未按 Shift 时使用标准、悬浮、按压中前三帧；按住 Shift 时使用同顺序的后三帧 | 按键状态实时决定帧组，动作仍只在原按钮内松开左键时触发 |
| 模型管理按钮、贴图 4 段 | 标准、鼠标悬浮、左键按压中、选中 | 选中且鼠标悬浮时复用第二帧；只在按钮内松开左键时切换选中状态 |
...
|---|---:|---|
| 标题栏 | `[0, 0, 349, 13]` | 默认以灰字居中绘制“塑料成型舱”；操作拒绝、载入/存入覆盖确认和文件反馈在该区域限时改用红色粗体显示；长文本等比缩小后仍按实际字高上下居中；当前背景没有独立关闭按钮，沿用容器界面的关闭键 |
...
| 指定类型 | `[9, 114, 16, 16]` | 装饰外框为 `[5, 110, 24, 24]`；贴图 4 段依次为标准、悬浮、按压中、叠加层打开；按钮中心内容区为 `[11,116,12,10]`，由原 `12x12` 中心区去掉底部两行得到；当前指定类型的立体物品图标按 `10x10` 渲染在该内容区的中央，即 `[12,116,10,10]`，左键按压中整体下移 `1 px` |
...
| 元素按钮 1 至 4 | `[268+32c, 82+23r, 32, 23]`，`r,c=0..1` | 顺序为左上、右上、左下、右下；cube 使用 `cube.png` 的三段 `32x23` 贴图，依次为标准、悬浮、按下/选中，按钮只显示 cube 名称，Tooltip 显示完整名称及继承父分组后的显示和锁定状态；分组继续使用两段按钮 |
...
| 加载磁盘 | `[265, 162, 16, 16]` | 使用 `load.png` 的三段 `16x16` 贴图；按第 10 节把当前结构磁盘中的模型载入建模区域，只在按钮内松开左键时执行 |
| 存入磁盘 | `[265, 180, 16, 16]` | 使用 `save.png` 的三段 `16x16` 贴图；按第 10 节把当前模型存入结构磁盘，只在按钮内松开左键时执行 |
| 磁盘过滤槽 | 物品格 `[287, 170, 18, 18]`，装饰外框 `[284, 167, 24, 26]` | 只接受 AnvilCraft 结构磁盘，不接受普通磁盘或其他物品，不对普通物流暴露 |
| 磁盘流向箭头 | `[308, 173, 12, 13]` | 非按钮，只表示磁盘与模型管理之间的模型流向 |
| 模型管理 | `[324, 171, 16, 16]`，装饰外框 `[320, 167, 24, 26]` | 使用 `model.png` 的四段 `16x16` 贴图，依次为标准、鼠标悬浮、左键按压中、选中，选中且鼠标悬浮时复用第二帧；中心 `12x10` 内容区独立渲染模型方向立方体，左键按压中整体下移 `1 px`；在按钮内松开左键时切换右叠加层，不等同于磁盘载入 |
...
2. 枢轴按钮第一次点击选择枢轴工具；已有 cube 被选中时再次点击执行“枢轴居中”，形成一个可撤销命令。多选的所有操纵器统一位于所选 cube 精确变换后整体外接范围的中心，拖动会同时操作全部选中 cube。移动为每个 cube 及其有效枢轴应用同一个固定世界轴位移且不改变旋转；旋转仍为每个 cube 绕各自有效枢轴应用同一个轴角。多选执行“枢轴居中”时，所有选中 cube 的有效枢轴统一设到上述共同中心，随后旋转必须让它们围绕该共同中心并保持相对位置。镜像按钮按标准、X、Y、Z 循环并与视口中实际选择的镜像轴保持一致，镜像结果越界时按第 3.2 节拒绝提交。所有工具按钮只在左键松开且指针仍位于原按钮内时执行状态或编辑操作。
...
5. 元素按钮按左上、右上、左下、右下排列，每屏四项；鼠标滚轮和右侧滚动条按行滚动。按钮列表采用 Windows 文件选择逻辑：普通左击单选并更新范围锚点，`Ctrl` 左击切换单个按钮且保留其他选择，`Shift` 左击用锚点到目标的连续范围替换当前选择，`Ctrl+Shift` 左击则把该连续范围加入当前选择。视口左击选择元素的既有 `Shift` 多选逻辑不变；视口右击 cube 时先选择该 cube 再打开上下文菜单，右击已在多选中的 cube 保留当前多选。视口选择元素时对应列表按钮进入按下态并自动滚到可见范围，点击列表项时视口选择和操纵轴同步更新。右击 cube 按钮时先单选对应 cube、刷新属性，再在建模区域最右侧打开与视口右击相同的上下文菜单；菜单提供重命名，零厚度 cube 使用相同逻辑。分组和“新建 cube”按钮不响应此右击操作。cube 按钮 Tooltip 第一行显示完整名称，后两行分别显示继承父分组后的“已显示/已隐藏”和“已解锁/已锁定”，不重复显示类型、零厚度能力或操作说明；这些能力由本手册说明。
...
11. 塑料熔体条悬浮提示至少显示 `暂存量 / 8 B`、`成型区域批次量 / M(V)` 和当前 `250 mB/gt` 泵送/暂停原因。FE 条悬浮提示显示精确 FE、最大 FE、`256 kW` 工作等级和暂停原因；最大容量引用当前版本超级电容器的同一常量，目标值为 `160 MFE = 160000000 FE`。
12. 左下资源槽接受塑料熔体桶、已充电普通电容器和已充电超级电容器。每 gt 最多处理一个容器或电容器；熔体桶完整倒入 8 B 暂存罐，普通电容器与超级电容器分别只在 FE 缺口至少为 `8000000` 或 `160000000` 时完整充入。输入栈耗尽时空容器留在槽内；输入栈仍有剩余时空容器返回当前操作玩家背包，背包满则掉落在成型舱旁。
13. 加载/存入按钮与磁盘槽按第 10 节工作；无磁盘时二者禁用。“模型管理”不依赖磁盘，只切换右叠加层。磁盘模型、共享 JSON、当前草稿和 8 B 暂存熔体是彼此独立的数据，不得因打开叠加层或载入模型而清空熔体。
14. 打开成型舱界面时 JEI 的物品列表和书签侧栏都不显示；离开界面后由 JEI 按玩家原有全局显隐状态恢复，成型舱不能改写该设置。
15. Jade 只追加一行“状态：”，并把等待粘土球、等待供电、等待熔体、可以加工等当前原因合并写入该行；资源显示复用通用能力组件，只保留电网条、熔体条、FE 红色分格条以及粘土球图标和数量，不再额外显示 FE 精确值、工作等级、暂存/批次数值、泵送速度或粘土分项等文字行。

#### 3.3.4 屏幕边界和仍缺的 GUI 资源

1. 布局、居中位置与缩放只按当前可见范围计算：两侧都关闭时使用 `[0, 0, 349, 226]`，仅左侧展开时使用 `[-72, 0, 421, 226]`，仅右侧展开时使用 `[0, 0, 421, 226]`，两侧同时展开时使用 `[-72, 0, 493, 226]`。只要完整范围能以每个逻辑像素对应整数个 framebuffer 像素的倍率显示，两侧就允许同时展开；当前窗口连最低整数倍率都无法完整容纳时，展开新的一侧自动关闭旧的一侧，窗口缩小时保留最后展开的一侧。绘制、命中和离屏视口必须使用同一变换，不能单独挤压建模视口或让叠加层按钮出屏。
2. 当前三张背景已经固定本节全部现有矩形。TODO-01 已按已知帧规则提供紫黑占位控件图集；正式资源只能原位替换，不得改变单帧尺寸、帧数、排列方向或命中矩形。后续生产资源条和槽位前景仍由对应 TODO 固定 UV。
3. 锁定按钮的第四帧归属尚未说明，最终实现版必须给出明确帧序，不能由实现者按代码方便程度决定。
4. TODO-01 中没有独立按钮的建模操作使用视口上下文菜单和既定键盘命令；当前操作者、制造分析与失败原因使用标题栏 Tooltip。后续若增加可见入口，必须先更新背景和本节坐标，不得占用既有矩形或塞入建模视口。
5. “重置模型”和锁定按钮的正式状态仍属于后续资源，不阻塞当前占位实现。“上传模型”不是独立按钮，而是通过第 3.5 节既有的同一入口按住 Shift 打开模型文件夹、放入文件后再普通点击刷新完成。
...
2. 叠加层内部继续使用左上角为 `(0,0)` 的半开矩形坐标：
   ...
   | 类型列表可视区 | `[4, 18, 52, 105]` | 显示七行 `15 px` 高的类型按钮；超出范围时按行纵向滚动 |
   | 类型滚动槽 | 外框 `[59, 17, 6, 110]`，交互轨道 `[60, 18, 4, 108]` | 背景不移动，轨道顶部向上延长 `2 px`；滑块按注册类型总数计算并使用 `slider.png` |
   | 普通 | `[4, 18, 52, 15]` | 默认选中；正式名称使用本地化文本 |
   | 箱子 | `[4, 33, 52, 15]` | 本期可选类型 |
   | 储罐 | `[4, 48, 52, 15]` | 本期可选类型 |
   | 铁砧 | `[4, 63, 52, 15]` | 本期可选类型，图标使用原版铁砧 |
   | 支架 | `[4, 78, 52, 15]` | 当前可选类型，内部稳定类型 ID 为 `anvilcraftplasticraft:tray` |

   每行最左侧 `15x15` 范围内按 `[6, 20+15r, 10, 10]` 渲染图标；该区域位于原中央 `11x11` 范围的左上角，即只从右侧和底部各缩小 `1 px`。名称从该 `15x15` 范围之后开始且位置不变。`r=0..4` 对应表中五行并全部实际填充，铁砧行使用原版铁砧图标；支架正式图标完成前复用普通类型的白色 `16x16x16 px` 标准通用塑料块。未来类型继续由注册表生成并使用滚动列表。
3. 搜索框只有被鼠标点击并取得焦点后才接收键盘输入；未取得焦点时，玩家键入内容不得进入搜索。取得焦点后每次文本变化都实时更新可见类型，不需要按回车；关闭叠加层、按 Esc 或点击界面内任何不属于搜索框的位置都立即结束输入。
4. 每个类型行使用 `category_entry.png` 的三段按钮贴图，依次为标准、鼠标悬浮、选中。类型选择是单选；点击一项会松开原选中项并选中新项。行左侧图标区独立渲染对应类型的立体物品图标；普通类型使用白色 `16x16x16 px` 基础通用塑料块，箱子使用原版箱子，储罐使用 AnvilCraft 流体储罐，铁砧使用原版铁砧，支架暂时与普通类型使用同一标准通用塑料块。主界面“指定类型”按钮也在四态背景之上独立渲染同一类型图标，类型图标不占用按钮背景帧。
5. 选择类型只更新当前模型的指定类型并立即请求服务器重新验证，不会锁定或加工模型。普通制品始终有效；箱子、储罐、铁砧或支架验证失败时保留玩家选择和编辑状态，显示“这个形状好像不能做这件事”及具体原因，并在修正形状前禁止锁定。
6. “指定类型”按钮再次点击关闭叠加层。每次新打开成型舱 UI 时叠加层默认关闭，但当前指定类型继续保存在模型中；重新展开时应自动滚到并显示当前选中行。
7. 左侧类型叠加层和右侧模型管理叠加层分别由各自按钮控制，打开或关闭任一侧都不会修改模型、选择或撤销历史。当前窗口能以整数 framebuffer 倍率完整显示两侧时允许同时展开，否则展开新的一侧会自动关闭旧的一侧；展开状态都不持久化。

### 3.5 模型管理叠加层

1. 背景使用 `plastic_molding_chamber_right.png`，尺寸 `73x131`，以主界面本地坐标 `(348, 72)` 绘制。它占用主界面坐标 `x=348..420`、`y=72..202`；左指尖像素与“模型管理”按钮中心连接。
2. 叠加层内部继续使用左上角为 `(0,0)` 的半开矩形坐标：
   ...
   | 模型滚动槽 | 背景外框 `[64, 17, 6, 93]` 不变，交互轨道 `[65, 18, 4, 89]` | 文件超过六个时纵向滚动；轨道相对旧实现向上延长 `2 px`、向右移动 `1 px`，滑块固定使用 `anvilcraftplasticraft:textures/gui/button/plastic_molding_chamber/slider.png`；该 `8x12` 贴图横向排列标准、悬浮两个 `4x12` 半幅且不拉伸，无法滚动时保持标准帧且不绘制禁用灰层 |
   | 置顶模型 | `[10, 112, 13, 13]` | 使用 `pin_to_top.png` 的三段 `13x13` 贴图，只在按钮内松开左键时执行 |
   | 复制模型 | `[25, 112, 13, 13]` | 使用 `copy_small.png` 的三段 `13x13` 贴图，只在按钮内松开左键时执行 |
   | 刷新模型/打开模型文件夹 | `[40, 112, 13, 13]` | 使用 `file.png` 的六段 `13x13` 贴图；未按 Shift 时使用前三帧并刷新模型，按住 Shift 时使用后三帧并打开模型文件夹，Tooltip 跟随当前按键状态，只在按钮内松开左键时执行 |
   | 删除模型 | `[55, 112, 13, 13]` | 使用 `delete.png` 的六段 `13x13` 贴图；未按 Shift 时使用前三帧且仅提示操作要求，按住 Shift 时使用后三帧并直接提交删除，只在按钮内松开左键时执行 |
...
4. 每个模型行使用三段按钮贴图，依次为标准、鼠标悬浮、选中。列表选择是单选；点击一项会选中该共享模型，并按第 10 节把它写入当前结构磁盘，但不修改成型舱中的模型。没有结构磁盘时仍允许保留列表选中态并提示先放入结构磁盘；结构磁盘已有不同模型时先显示覆盖确认，确认前不改磁盘。视图滚动后选中态不得丢失。
...
8. 未按住 Shift 点击删除按钮只提示“按住 Shift 点击以删除所选模型”，不维护二次点击确认态；按住 Shift 点击并在按钮内松开后直接提交删除。服务器再次检查 owner/管理员权限和文件修订号；删除 JSON 不清除已经写入磁盘的内嵌副本，也不影响已载入成型舱的模型。
9. 没有选中模型时，置顶、复制和删除按钮禁用，打开模型文件夹保持可用。搜索、滚动、按钮确认和当前选中项只是界面状态，不得被伪造网络包用来绕过服务端文件权限。
10. 叠加层不替换建模会话。展开或关闭不会提交、回滚、锁定或清空当前模型；每次新打开成型舱 UI 时默认关闭，再次点击“模型管理”关闭，不记忆上次开关状态。
...
1. 粘土填充当前制造烘焙外接空间中不属于有体积塑料模型的空间，包括制品内腔。一粘土球等于 `1024` 立方像素；设外接空间共有 `B` 个立方像素、模型实体积为 `V`，每次锁定需要：
...
5. 普通 `48x48x48 px` 工作空间中 `B = 110592`，理论最大需求为 108 球，默认 `256` 上限可以一次容纳完整批次；超限模型按其整数外接尺寸计算 `B`。玩家主动把上限调到需求以下时，机器仍按已有进度暂停并允许物流继续补料。GUI 状态信息显示“槽内数量 / 已成模数量 / 总需求”，锁定按钮确认态显示“槽内数量 / 总需求”；自动化测试必须覆盖自定义低上限时的填充中补料、256 球实际存取和区块重载。
...
1. 模型实体积记为 `V` 立方像素。成型区域本周期可容纳的熔体上限为：
   ...
   制造换算固定为 `1 mB = 4` 立方像素，除法结果向上取整。普通 `48x48x48 px` 最大实心模型的 `V = 110592`，因此成型区域最多需要 `27648 mB = 27.648 B`。

2. 成型舱本体另有固定 `8 B = 8000 mB` 暂存罐。它和成型区域是两个独立容量：暂存罐最多 8000 mB，成型区域最多 `M(V)`，两者同时装有熔体时允许总量达到 `8000 + M(V)` mB；普通最大实心模型对应的总量为 `35648 mB`。不得用 `M(V)` 限制暂存罐，也不得把暂存量算作已经填入模型。
...
4. 机器只有在模型已锁定、粘土模具全部填充、结构完整且有效供电时，才把暂存罐中的熔体泵入成型区域。泵送速率固定为每游戏刻 `250 mB/gt`，每刻实际转移量为 250 mB、暂存量和成型区域剩余容量三者的最小值；暂存空间一释放，外部管道即可继续补入。
...
12. 部分结果在加工前重新运行功能类型算法。若不再满足箱子、储罐、铁砧或支架条件，则允许加工，但自动降级为普通制品；GUI、Jade 和铁砧锤浮窗要提前显示“本次结果将降级”。
13. 成品继承成型区域本批实际熔体的类型、颜色和相关材料组件，而不是读取加工时仍留在 8 B 暂存罐中的流体。
14. 收到落砧加工事件后立即暂停泵送，并以事务开始时已经位于成型区域中的熔体冻结加工快照；不得绕过 `250 mB/gt`，把暂存罐余量瞬间提交到本批。加工成功只消耗并清空成型区域熔体，8 B 暂存罐中的剩余熔体数量和身份完全保留，可供以后批次继续使用。
15. 暂存罐中有熔体不妨碍编辑、重置模型、手动解锁或从磁盘载入模型。只有成型区域内已经存在批次熔体时，才拒绝解锁、重置或覆盖模型，并在 GUI 标题栏用红字提示先从顶部或底部接口抽空成型区域；这些操作不得自动删除任何一处熔体。
16. 内部调试覆盖不向成型区域泵送熔体，也不要求 250 mB 最低批次。输出预检完成并进入加工事务时消耗资源槽中的 `1` 个调试资源；加工失败、中止或之后解锁均不返还，暂存罐与已有批次熔体都不被本次加工结算。
17. 破坏成型舱时，8 B 暂存罐中的熔体不生成掉落物，也不写入掉落的成型舱物品或其他容器，随被破坏的方块实体直接舍弃。粘土球和磁盘仍分别按第 4.2 节与第 10 节的物品规则掉落。
...
2. 制品数据至少保存：格式版本、实际成型形状、模型哈希、材料、颜色、功能类型、容量、名称、箱子物品、储罐流体或支架承载元件及运行状态；实体另行保存朝向面和面内四分之一转角，物品中的朝向字段固定规范化为默认方向。
...
6. 包括内部调试覆盖超限制品在内的动态实体要求现有塑料物理、载具碰撞、路径规划和宽阶段检索全部取消固定 1x1x1 与 3x3x3 尺寸假设。
7. 极端长薄结构、断开部件、纯零厚度 cube、256 cube 模型和最大实心模型都要有性能预算；若确实需要复杂度上限，应在导入/锁定前拒绝并说明，不能改变已经生产的形状。

### 7.3 类型驱动的物品 Tooltip

1. 成型制品的物品与实体显示名由塑料材料本地化名称和最终功能类型后缀共同组成，不使用蓝图或模型名称。普通类型后缀为“块”，其他类型默认复用类型注册项的本地化名称；例如通用塑料依次命名为“通用塑料块”“通用塑料箱子”“通用塑料储罐”“通用塑料铁砧”“通用塑料炼药锅”“通用塑料大型炼药锅”“通用塑料支架”。未来新增塑料材料和功能类型继续使用同一组合规则。
2. 成型制品的物品 Tooltip 只根据成品最终保存的功能类型和派生总容量选择，不由客户端重新扫描形状，也不使用加工前的类型选择。明确选择普通制品、没有指定功能类型以及部分加工后降级为普通制品的结果都使用普通文案。
3. `zh_cn` 的非 Shift 单行 Tooltip 固定如下；其他语言提供语义对应的本地化文本，不能在代码中硬编码中文：

   | 成品最终类型 | Tooltip |
   |---|---|
   | 未指定或普通制品 | `一块普普通通的塑料` |
   | 箱子 | `一个塑料箱，能储存 %s 组物品` |
   | 储罐 | `一个可储存 %s B 流体（含熔岩）的塑料储罐` |
   | 铁砧 | `一个能执行落砧配方的塑料铁砧` |
   | 炼药锅 | `一个可储存 %s B 单一流体（含熔岩）的塑料炼药锅，可被点燃并执行落砧配方` |
   | 大型炼药锅 | `一个可分八层共储存 %s B 流体（含熔岩）的大型塑料炼药锅，落砧配方具备多倍效率` |
   | 支架 | `一个可承载红石中继器、比较器、红石火把、脉冲发生器和物品探测器的塑料支架` |

4. 文案中的 `%s` 是本地化格式参数，不是需要原样显示的字符。箱子传入第 8.3 节派生的物品格数，储罐传入第 8.4 节派生的整数 `B` 容量，炼药锅传入第 8.7 节派生的整数 `B` 容量（大型炼药锅固定为 `512`）；三者都显示制品的总容量，不随当前已存内容变化。现有未指定类型的通用塑料块继续使用同一句 `一块普普通通的塑料`，不得另造普通制品文案。
5. 铁砧 Tooltip 不带容量参数，只说明其落砧配方能力；支架 Tooltip 不带容量参数，完整列出五种可承载元件。所有塑料储罐与炼药锅均可储存熔岩，耐热塑料仍按其材料特性免疫火焰。部分成型若不再满足对应验证器则降级为普通制品。箱子和储罐物品在原有基础 Tooltip 之外，还要在指针正上方显示独立的内容浮窗；该浮窗使用原版默认 Tooltip 边框，不使用药盒或护符盒的专用贴图。箱子首行“已用格数”和储罐首行“已有流体”为白色，其余逐项内容为淡灰色，所有文字均保留默认深灰色阴影。炼药锅同时有分层流体和加工槽位，两段内容都要列出，槽数取自第 8.7 节的槽位布局而非容量。箱子当前内容也由 Jade 和铁砧锤浮窗展示；储罐在 Jade 中只使用其流体能力提供的默认流体条，铁砧锤浮窗仍显示流体总量和逐项内容。铁砧锤浮窗的物品、流体逐项内容文字同样使用淡灰色并保留深灰色阴影。

## 8. 制品类型、容量和无 GUI 存储

### 8.1 可扩展类型框架

1. 类型系统使用注册表/策略接口，每个已实现类型提供稳定 ID、成品名称后缀、形状验证器、失败原因、容量派生、物品 Tooltip、物品/流体能力、Jade/铁砧锤浮窗内容和数据迁移。接口必须允许未来加入新类型，但不得为未定义功能提供猜测性默认实现。
2. 普通制品永远通过。箱子、储罐、铁砧和支架在选择、锁定和实际部分加工时均由服务器重新验证。
3. 验证失败统一提示“这个形状好像不能做这件事”，并附加具体原因。
4. 资源槽中的多相超限合金只打破类型验证，不能打破 `3x3x3` 尺寸限制；只有所选类型原本无效时，第二次点击确认锁定才消耗 `1` 个，之后解锁不返还。内部调试覆盖同时打破类型、尺寸和熔体限制，确认锁定时保留调试资源，真正开始加工时才消耗 `1` 个，之后失败或解锁不返还。连续模式每周期重新验证并消耗本周期需要的覆盖资源。
5. 任一覆盖强制类型时，按模型完整整数外接总体积计算最大属性，不扫描真实内腔；箱子容量仍按每 `64` 立方像素 `1` 格向下取整，储罐仍按每 `171` 立方像素 `1 B` 向下取整。例如实心 `8x8x8 px` 模型用多相超限合金强制指定箱子时得到 `8` 格。
...
6. 箱子、储罐和炼药锅的完整模型验证都允许反向绕序的有体积 cube（奇数个负尺寸轴，或经变换后等效反向）作为空腔或壁。验证先将其保留为壁，再逐个尝试挖为空腔；只有这样会保持并扩展相应的密闭或开顶内腔时才采用。因此一圈壁内部的空腔即使被此类 cube 全部或部分填满，填入部分仍是可用内腔；不满足该条件的反向 cube 继续作为壁。
...
5. 大容量浮窗必须分页/滚动或按相同物品汇总，不能一次发送/绘制全部空槽。实体回收为物品时保留库存，并防止递归容器数据造成无限嵌套。超大容器把权威内容放在世界级 SavedData，物品与实体只持 UUID 和有界摘要。
...
4. 塑料实体储罐没有 GUI。它像 AnvilCraft 大型流体储罐一样，在同一个共享总容量中保存多种完整 `FluidStack`，支持按流体及组件精确输入/抽出。Jade 只使用实体流体能力生成的默认流体条，不额外重复总量或逐项文本；铁砧锤浮窗显示流体列表、各自数量和总容量。
5. 通过多相超限合金或内部调试覆盖强制制成的储罐仍保留完整容量和全部流体能力，但不生成内部液体体积与自由液面渲染；Jade、铁砧锤浮窗和存取行为不受影响。
6. 储罐实体回收为物品后保留全部流体；放回世界后恢复相同顺序、数量和组件。
7. 对外能力、手持流体容器交互、输入选择和抽取顺序尽量直接复用本体大型流体储罐语义；不能让渲染层顺序反过来修改服务端保存顺序。真正销毁且无法保留为物品时，按本体约定安全排出或丢弃并给出明确反馈，不得数据无声消失。

### 8.5 铁砧

1. 铁砧验证固定使用模型世界 Y 轴，最低占用层就是下底面；不扫描任意旋转方向，也不把侧面或顶面当作底面。
2. 最低层必须包含连续、无孔的至少 `12x12 px` 平整矩形，并保持整个模型沿 Y 连续；相邻占用层必须有重叠，不能只在角上接触。
3. 验证器寻找三段连续分界：底部厚度至少 `3 px`，中部厚度至少 `3 px`，顶部厚度至少 `4 px`，总厚度至少 `10 px`，且顶部严格厚于底部。
4. 中部段的整体 Y 投影必须完全落在下底面投影内，投影多边形的四个边界都严格内缩，不能有任何一面与下底面相等；顶部段的整体投影面积必须严格大于下底面面积，允许部分边界相等，只比较总面积。
5. 选择铁砧后，服务器在选择、锁定和实际加工前都重新运行上述扫描；欠料导致的形状不再满足规则时，照常加工但最终类型降级为普通制品。
6. 完整成品保存铁砧功能：只有局部下底面朝世界下方并以该底面落地时才发布 AnvilCraft `AnvilEvent.OnLand`，默认 `HasAnvil` 谓词可匹配并执行落砧配方；侧面、顶面或倒置撞击不发布，专用属性/NBT 谓词不被放宽。右击实体或使用铁砧锤可打开共用铁砧菜单，粘附方块态也保持同样有效性。
7. GUI 为每种塑料铁砧使用独立的两层 `256x256` 纹理并绘制 `176x166` 可见区域：`{material}_plastic_anvil_base.png` 是按材料显示规则着色的底图，`{material}_plastic_anvil_overlay.png` 是不透明覆盖图；透明塑料在覆盖图上额外叠加低强度的对应色板颜色，既保留颜色辨识度也不改变覆盖图 alpha；当前材料为通用、工程、耐热和透明塑料。

### 8.6 支架与托盘能力

1. 支架验证固定使用模型世界 Y 轴。最低占用层是下底面，最高占用层是顶面；不扫描另外五个方向，也不因模型旋转而把侧面当作底面。
2. 全部可见元素必须位于建模区中心一格的 `x/z=[16,32)`，不是只限制总投影面积为 `16x16 px`；从最低表面到最高表面的占用高度最多为 `4 px`。
3. 下底面必须由有体积 Cube 构成，并形成共面、四邻接连续且无孔洞的像素区域；零厚度 Cube 不能充当底面。底面自身的全部厚度属于底，不视为底面另一侧的其他元素；真实最低 Y 下方不能再有其他元素。
4. 最高占用层必须存在完全水平的真实顶面，其像素投影与下底面完全一致，真实最高 Y 上方不能有其他元素。验证使用源 Cube 的精确表面确认真实顶底水平，不能让绕 X/Z 旋转产生的斜面仅凭首末制造体素层通过；绕 Y 旋转且顶底仍水平的模型允许通过。
5. 顶面和底面的中心 `x/z=[23,25)`，即中心 `2x2 px`，必须同时完整覆盖。任一面缺失中心、底面断开或有孔、顶底投影不一致、越出中心一格或高度超过 `4 px` 都拒绝指定支架类型。部分成型后重新运行同一验证，不再满足时保留实际形状并降级为普通制品。
6. 合法支架成品具有单元件托盘能力。手持红石中继器、红石比较器、红石火把、AnvilCraft 脉冲发生器或物品探测器右击空支架即可放入；潜行并空手右击可取回，物品组件和方块实体配置随支架物品、动态实体、粘合方块态及区块重载完整往返。
7. 承载元件按原方块模型渲染在塑料实体顶面，并保留原交互：中继器可调延迟、比较器可切换模式，脉冲发生器和物品探测器可打开原菜单并使用结构磁盘。取回时保留配置但清除仅属于本次放置的输出、相位和倒计时状态。
8. 支架动态实体即使移动且不对齐方块网格，也以当前精确几何的六个表面格发布和读取红石信号。局部前后左右上下随 24 种姿态映射到世界六向；中继器、比较器、红石火把、脉冲发生器和物品探测器保持各自输入、输出、侧输入、模拟输入、延迟、脉冲与探测范围语义，粘合方块态使用同一运行时和端口映射。
9. 支架类型行和主按钮的正式图标尚未完成；当前固定复用普通模式的白色 `16x16x16 px` 标准通用塑料块，后续只替换图标资源，不改变类型 ID、行坐标或行为。

### 8.7 炼药锅与大型炼药锅

1. 炼药锅是开顶容器，不能用第 8.2 节的密闭内腔洪泛判定：开顶内腔与外界连通，六邻接洪泛只会得到空腔。它使用独立扫描——先在每个 `y` 层做 2D 水平洪泛判断哪些格被侧壁围住，再要求内腔格下方是实心或已属内腔。侧壁不等高、侧面漏孔、圆形和异形轮廓都由这两步自然覆盖，模型是圆是方都不影响判定。
2. 形状要求：底面与四周侧壁完整保留，仅顶部形成开放开口，其余边界封闭。验证固定使用模型世界 Y 轴，不扫描另外五个方向。
3. 顶面开口按**像素面积**判定，不按外接盒也不按内切正方形：开口面积为「内腔顶格正上方为非实心且可达外界」的不同 `(x,z)` 列数，至少 `144` 平方像素（相当于 `12x12`）。原版炼药锅形状开口正好 `144`，刚好合格。
4. 内腔整体深度取 `maxY + 1 - minY`，至少 `8 px`，即最矮的一侧壁高不低于 `8 px`。用整体深度而不是逐列高度，否则碗底和弧形底会被误判。
5. 验证失败原因分三类：内腔为空或未封闭 `cauldron_not_sealed`、开口不足 `cauldron_opening_too_small`、深度不足 `cauldron_too_shallow`；大型炼药锅额外有开口未超阈值 `cauldron_not_large`。
6. 容量以原版炼药锅内腔 `12x12x12 = 1728` 立方像素为 `1 B` 基准，公式为 `max(1, floor(内腔体积 / 1728))`，只产生**整 B**，不出现 mB 零头。因此 `1727`、`1728` 和 `3455` 立方像素都是 `1 B`，`3456` 才是 `2 B`，`5183` 仍是 `2 B`，`5184` 才是 `3 B`，依此类推。
7. 开口面积**大于** `1600` 平方像素时自动升级为大型塑料炼药锅，容量固定为 `512 B`，不再按内腔派生。升级不算降级：类型面板与生产提示只对「最终类型既不等于请求类型、也不是请求类型的升级结果」显示降级文案。
8. 槽位与流体分层由统一的槽位布局给出，实体态、粘合方块态和物品能力共用同一套槽号（先输出槽后输入槽）：

   | 形态 | 输出槽 | 输入槽 | 输入槽堆叠上限 | 流体层数 | 单层容量 | 单次落砧配方通道 |
   |---|---|---|---|---|---|---|
   | 炼药锅 | `8` | `8` | 原版堆叠上限（通常 `64`） | `1` | 整锅容量 | `1` |
   | 大型炼药锅 | `32` | `8` | 原版堆叠上限 × `9`（通常 `576`） | `8` | `64 B` | `9` |

9. 流体列表索引 `0` 为最底层、末尾为最顶层：填充优先合并同种流体，层位用满后拒绝新流体；抽取自顶向下。点燃、燃料和配方查询一律按最底层流体作答，对齐本体大型炼药锅的 `index 0` 与 `bottomAccess()` 语义。普通炼药锅只有一层，因此实际表现为单一流体。
10. 功能与硬化树脂炼药锅一致：可被点燃、点燃后的流体伤害其中实体、注册进流体网络、吸收接触物品并按出口方向弹出、参与油气催化与等离子喷流、倒扣时掉出内容、铁砧锤可回收连同内容。落砧配方由**普通铁砧**即可触发，不要求巨型铁砧。
11. 大型炼药锅的多倍效率表现为单次撞击最多跑 `9` 个配方通道。每个通道各自挑一条仍能匹配的配方，本轮没有推进锅内物品或流体状态就立即收手，避免空转。输入槽是否为空只在撞击瞬间判定一次，多个通道共用同一个加工作用域，锅不会把自己刚产出的东西在同一刻当作输入反复回炉。普通炼药锅只有 `1` 个通道，行为与硬化树脂锅完全一致。
12. 所有塑料炼药锅都可装入熔岩，锅内熔岩不会销毁制品；非耐热制品接触世界中的熔岩仍按外部热害销毁。
13. 锅内的塑料熔体对落入其中的实体施加蜘蛛网式减速，实体态与粘合方块态都生效；减速判定只对炼药锅类型生效，装着同种熔体的储罐不受影响。
14. 内腔掩码随制品数据保存，锅内流体沿用第 9 节储罐的液面求解与分层裁切，任意形状的锅都能正确渲染液面，无需另写渲染路径。
15. 创造物品栏只提供普通炼药锅的示例蓝图（原版炼药锅形状，即硬化树脂炼药锅形状）× 四种塑料材料；大型炼药锅不进创造物品栏，只能由玩家自制开口超过 `1600` 平方像素的模型升级得到。
16. 类型面板上炼药锅按钮的图标使用原版炼药锅物品，不使用示例模型，与铁砧、箱子等已有类型同一套写法；大型炼药锅不可主动选择，因此不在类型面板中显示。

## 9. 任意重力方向下的多流体渲染

1. 储罐的权威容量和流体列表沿用大型流体储罐的多流体语义；渲染只消费该数据，不能反向改变容量。
2. 强制突破限制制成的储罐不进入后续内部液体网格与液面渲染流程；这只影响显示，不改变权威容量和内容。
3. 从内腔单元和阻隔面生成密闭的“可装液体空间”网格。渲染网格要向内做极小偏移防止 Z-fighting，但不能越过实体或无厚度边界。
4. 每帧或重力变化时取得实体的有效重力向量。自由液面所在平面的法线与重力方向平行；重力可为斜向，因此液面必须能以任意角度倾斜，而不是只支持六个水平面。
5. 根据 `当前总流体量 / 总容量` 求应占内腔体积，再沿重力方向二分求自由液面平面，使平面“下方”与内腔相交的体积恰好等于目标体积。
6. 多种流体按照储罐保存的稳定顺序分层。为每种流体计算相邻两个等体积平面，把内腔网格裁切成对应斜向薄层；各层不能重叠，也不能在应填充区域留下孔洞。
7. 凹形、带细管、多个独立内腔、零厚度密封面和任意复杂合法储罐都使用同一半空间裁切算法。满罐时必须填满全部内腔，不穿模；未满时只允许自由液面以上存在预期空区。
8. 多个内腔按第 8.2 节视为一个连通逻辑储罐，统一受同一组重力平面切割。
9. 重力方向连续变化时先对有效重力方向做短时间插值，再用插值后的方向重新求满足目标体积的液面位置；不能直接插值两个裁切平面而造成液体体积忽多忽少。重力接近 0 时保持最后一个稳定方向。
10. 半透明流体层需要按相机和各层几何正确排序，或使用项目可承受的顺序无关透明方案；从罐体任何一侧观察都不能出现后层覆盖前层、层间闪烁或把流体画到外壳前面。
11. 缓存内腔拓扑和裁切中间数据，只在模型、流体数量层级或重力方向超过阈值变化时重建网格。必须为 0°、30°、45°、90°、连续旋转和最大复杂内腔建立渲染验证样例。

## 10. 磁盘、JSON 和共享蓝图库

1. 复用 AnvilCraft 本体结构磁盘物品，并在其 `DISK_DATA` 中嵌入成型蓝图；成型舱不再实现普通磁盘兼容接口，也不新增重复蓝图物品。成型蓝图与本体 `STRUCTURE_DISK_DATA` 互斥，普通方块结构磁盘仍完全走 AnvilCraft 原逻辑。
2. GUI 按第 3.3 节固定坐标包含结构磁盘专用格、格子左侧上半的“加载磁盘”、左侧下半的“存入磁盘”，以及格子右侧的“模型管理”按钮。无结构磁盘时存入/加载禁用；只要槽内有结构磁盘，“存入磁盘”就可点击，不要求模型先锁定。“模型管理”不依赖结构磁盘，只负责切换第 3.5 节叠加层。
3. 任意机器阶段都可点击“存入”，把当前模型和所选功能类型写入结构磁盘，并把标准 JSON 保存到世界共享蓝图库。空结构磁盘一次点击直接完成；已有方块结构、成型模型或其他数据的结构磁盘第一次点击只在 GUI 标题栏红字显示“结构磁盘已有数据，再次点击确认存入”，第二次点击才原子替换磁盘并写入共享副本。文件所有者是实际点击“存入”的玩家，不是机器放置者。
4. 蓝图库对所有玩家可见和可载入。文件记录 owner UUID/名称；所有者和管理员可以覆盖、重命名或删除，其他玩家默认只能读取并写出自己的副本。
5. 模型管理叠加层只能由“模型管理”按钮展开或关闭。玩家在叠加层搜索并点击共享模型行时，该行进入选中态，并把规范化蓝图写入当前结构磁盘；没有结构磁盘时只保留本次 UI 会话的选中项并提示先放入结构磁盘，不修改成型舱。结构磁盘已有不同数据时必须先显示名称/哈希差异和覆盖确认，取消或写入失败均保持结构磁盘原数据。
6. 主界面“加载磁盘”只负责把当前结构磁盘模型加载到成型舱的可编辑状态，不承担从共享列表写结构磁盘的第一段操作，也不自动展开 JSON 叠加层。建模区域已有至少一个 cube 时，第一次点击只在 GUI 标题栏红字显示“有正在编辑的模型，再次点击确认载入”，第二次点击才清空建模区域并导入结构磁盘模型。关闭叠加层不清除本次 UI 会话中的列表选中项；无结构磁盘或结构磁盘没有有效蓝图时按钮禁用并说明原因。
7. 从结构磁盘载入只加载模型和可选类型建议，不直接锁定；玩家可以继续修改模型、重新选择类型，再手动或由生产模式锁定并消耗粘土。
8. 结构磁盘自身嵌入规范化蓝图和哈希，JSON 是共享副本而不是唯一数据源；共享文件丢失时结构磁盘仍可使用。
9. 原生 JSON 使用 UTF-8、版本字段、稳定字段顺序、原子写入和安全文件名。共享文件名允许中文等 Unicode 字母、数字和组合标记，同时继续拒绝目录分隔符、控制字符和路径穿越。至少保存名称、owner、工作空间坐标约定、元素/分组/变换、类型建议、时间、制造烘焙版本和模型哈希，不保存熔体、颜色、当前粘土或容器内容。
...
11. 支持零厚度、重叠、隐藏状态、负坐标和不从 0 开始的模型。任意受支持模型都允许导入共享蓝图库、写入结构磁盘并载入成型舱建模区域；三轴完整外接尺寸都不超过 3 个方块（即每轴 48 个 1 px 制造单位）时，允许玩家预览并确认整体平移到 3x3x3 工作空间内，不自动缩放；Blockbench 中从 `(0,0,0)` 开始、大小恰为 `48x48x48` 的模型必须自动以保持 `(0,0,0)` 数值的根变换贴合整个工作空间并直接识别。任一轴超出 3 个方块时保留原始尺寸载入并使用动态外接空间完成制造烘焙；玩家仍可编辑和查看，但“锁定并开始加工”禁用。导入后的源形状、超限载入和内部调试覆盖制造预览都要有往返样例验证。
12. 鼠标指向普通方块结构磁盘时，本体“结构/尺寸/智能方块放置器” Tooltip 和旋转方块预览保持不变。结构磁盘存有成型蓝图时，“结构”显示模型名，“尺寸”显示完整加工后可见几何的真实三轴格尺寸，最后一行改为绿色“可以被塑料成型舱加工”或红色“模型过大无法加工”；`80x80` 悬浮预览改为按真实外接范围居中、以本体同样的每 tick `2°` 速度旋转成型模型。塑料制品 Tooltip 使用同一组数值并统一显示“尺寸”。
13. 把已有成型蓝图的结构磁盘载入成型舱时，机器处于 `EDITABLE`、成型区域批次熔体为 0 且没有预留粘土即可覆盖，模型是否超过 `48x48x48 px` 不影响载入。8 B 暂存罐可以非空，载入模型不得清空或改写其中的熔体。当前建模区域非空时按第 6 条用标题栏红字二次确认；机器已成模、正在加工或成型区域仍有熔体时直接在标题栏说明原因，不能改写结构磁盘、当前草稿或机器资源。
14. 资源槽 Tooltip 固定为“放入电容器充电”“放入桶装塑料熔体加注”“放入多相超限合金可打破类型限制”三行。内部调试覆盖资源存在或已提交时，GUI 左侧 FE 条从深浅相间红色切换为深浅相间紫色；任一覆盖生效时，锁定按钮内沿显示亮紫色 1 px 方框和上浮紫色粒子，悬浮时变浅，按住左键时顶部下沉 `1 px`。
15. 向已有数据的结构磁盘“存入”、从共享模型行向已有数据的结构磁盘写入或用同名文件保存时必须显示覆盖确认，默认生成不冲突文件名。失败的文件写入不能改变结构磁盘原数据；结构磁盘写入和 JSON 原子替换要么都成功，要么保留可恢复的一致状态。
16. 服务器蓝图库位于 `<world>/anvilcraftplasticraft/blueprints/`；客户端只在玩家普通点击“刷新模型”时，从 Shift 打开模型文件夹所对应的安全目录上传白名单扩展名、受限大小和深度的文本。禁止路径穿越、符号链接逃逸、任意服务器路径读取和外部父模型解析；允许的父模型只能来自明确白名单并在导入时展开为自包含蓝图。
17. 手动刷新导入、成功回执后删除未变化的源文件、置顶、复制、Shift 打开模型文件夹和 Shift 二次确认删除的完整交互按第 3.5 节执行。复制和导入产生的新文件归操作者所有；置顶只改变操作者自己的排序；打开文件夹不能泄露远程服务器路径；删除始终由服务端复核权限。
...
| `MOLD_READY`         | 否              | 已达到总需求          | 完整 3x3x3 | 是           | 有效供电时固定 `250 mB/gt`，至 `M(V)` | 否，成型区域熔体不足  |
| `PROCESS_READY`      | 否              | 已达到总需求          | 完整 3x3x3 | 是           | 有效供电时固定 `250 mB/gt`，至 `M(V)` | 有效供电且结构合法时是 |
...
3. `MOLD_READY` 与 `PROCESS_READY` 只由成型区域熔体是否达到 250 mB 自动切换，8 B 暂存量不计入门槛。抽出成型区域熔体后可以从后者退回前者；每个正常游戏刻最多按 `250 mB/gt` 从暂存罐泵入，落砧事件不能触发额外转移。
...
5. 模型、互斥模式、红石上次电平、待锁定请求、动画进度、粘土槽当前堆叠上限、槽内/已成模粘土、8 B 暂存熔体、成型区域批次熔体、内部 FE、磁盘和等待原因都要分别持久化。加工结果不进入机器持久化数据，粘土动画结束后也没有额外等待状态。类型选择和模型管理两个叠加层的开关都不持久化，每次打开 UI 都从关闭开始。
...
> **实现状态**：TODO-01 至 TODO-06 及第 8.6 节支架托盘能力已按固定背景、坐标和占位资源进入实现与验收；正式模型、按钮和支架类型图标以后保持现有资源 ID、尺寸与帧规则原位替换。
...
**验收清单**：放置失败不留部分区域；控制器在区域前表面底部中央，GUI 视口在正确位置直接显示其方块模型；GUI 与世界均以灰色线标出 `48³` 在当前视角下的最外圈投影轮廓，轴向正视为 4 条、斜视为 6 条且不绘制轮廓内部棱，二者都只绘制 `Y=16` 的单层 `3x3` 参考网格，中心格为 1 px 实色细网格、外围八格无细分，并与世界 27 格使用同一非镜像坐标映射，蓝色参考轴从交点沿中心格边界朝 `+Z` 显示；全部建模线条在任意缩放下保持连续、平滑、固定屏幕宽度，交点与尖角没有缺口或层叠遮挡感，右上方向轴使用细线；世界线条和模型统一上移 `0.05 px`；GUI 中白色不透明、世界中整体半透明的精确源 Cube 旋转后保持连续斜面和逐面光照，四个机器朝向均保持相同正负绕序与上下表面明暗，世界投影响应局部方块光、天空光、遮挡与昼夜变化，制造体素只参与分析；主背景只使用 `349x226` 可见范围，左右 `73x131` 叠加层分别准确锚定在 `(-72,72)` 与 `(348,72)`，右侧底部四个操作按钮下移 `1 px`，其余叠加层元素及实际存在的 JSON 模型按钮保持原位，不为缺失模型绘制空按钮；默认北向左下，右上红/绿/蓝方向线和 W/E、U/D、N/S 标签只随相机正确变化，红色 X/WE、绿色 Y/UD、蓝色 Z/NS 不随机器朝向换轴或反向，单选旋转 cube 的六个缩放端点跟随 cube 自身及父分组姿态且不受机器朝向影响，多选缩放端点使用世界轴，线端与字母保留间距；左拖旋转的纵向手感与鼠标方向一致，右拖平移、左键短按选择、右键短按先选择命中的 cube 再打开上下文菜单，四者互不冲突；移动和枢轴工具快捷键分别为 `V` 和 `P`；cube 不能移出 3x3x3 工作空间；五张正式工具贴图及其按压、悬浮、选中与循环帧正确，五个工具互斥并允许标准空选状态，操纵器外观互异、保持固定屏幕尺寸并与命中代理一致，轴杆变细但末端不变，旋转三色环位于枢轴且隐藏背向相机部分并具有紧贴其外侧的完整白色外环；鼠标指向 Cube 或操纵轴时显示半透明白色反馈；四格元素列表与视口选择同步，Cube 按钮使用 `cube.png` 的标准、悬浮、按下三帧，以带黑色阴影的白色文字显示名称且 Tooltip 仅显示完整名称，右击 Cube 按钮先单选并在视口最右侧打开含重命名的同一上下文菜单；新建按钮使用 `new_cube.png` 的标准、悬浮、按压中三帧，只有松开时指针仍在按钮内才新建 Cube，移出后松开取消；撤销、复制、剪切、粘贴分别使用对应 `16x48` 正式贴图；主界面滚动柄只绘制 `slider.png` 中对应的 `4x12` 标准或悬浮半幅且不拉伸，交互轨道相对旧实现向右移动 `1 px` 并保持 `46 px` 长度，背景贴图不移动；末尾新建按钮在 `(16,16,16)` 交点产生 `(16,16,16)..(18,18,18)` 的 `2x2x2 px` cube，位置与枢轴均从 `(0,0,0)` 开始；Tooltip 不显示换行控制符方框且靠近屏幕边界时自动折行；四行十二个数值框分别显示“位置/大小/枢轴点/旋转 x/y/z”的简洁 Tooltip，数值属性居中并左移 2 px，最多 10 步撤销栈可用；搜索框文本和焦点规则符合第 3.4、3.5 节；打开界面时 JEI 隐藏且离开后不改变原设置；两段三帧与两段两帧按钮的悬浮行为符合第 3.3 节；无 UV/绘画页；区域 `getShape` 和碰撞均为空，投影近边贴合成型舱且进入区域后不被剔除；保存重开不丢元素；小窗口不发生控件覆盖；窗口化/全屏、高 DPI 和全部 GUI 缩放下视口清晰且命中一致；资源重载和反复打开关闭后不丢草稿、不残留 GPU 资源；发布物不包含浏览器运行时且建模器没有直接 OpenGL 调用。

**本项不包含**：粘土锁定、熔体、供电与生产；共享蓝图文件操作；动态成品或功能箱罐。

### TODO-02 磁盘、JSON 叠加层和多人共享蓝图库

**玩家可见结果**：玩家可以随时把当前模型存入结构磁盘，在默认收起的模型管理叠加层中按住 Shift 打开模型文件夹、放入文件并普通点击刷新完成导入，也可搜索、置顶、复制、定位或删除共享模型，再通过结构磁盘把任意受支持模型载入成型舱继续编辑；超限模型保留真实尺寸，但不能锁定加工。

**前置依赖**：TODO-01 和 TODO-03。本 TODO 使用 TODO-03 的真实机器状态保护载入覆盖，但“存入磁盘”不以锁定状态为前置。

**规格范围**：第 3.3、3.5 节和第 10 节。

**必须交付**：复用 AnvilCraft 结构磁盘及其 `DISK_DATA`，拒绝普通磁盘；第 3.3 节坐标中的结构磁盘专用槽、左上“加载磁盘”/左下“存入磁盘”按钮、共享模型行写入结构磁盘与加载按钮送入建模区的分离流程；结构磁盘内嵌蓝图与哈希、模型 Tooltip 和旋转悬浮预览；服务端共享 JSON 目录、owner/管理员权限与原子写入；第 3.5 节右叠加层最多六行且只按实际模型数绘制的列表、共用 `slider.png` 滑块、滚动和按钮帧交互；以同一个“刷新模型/打开模型文件夹”按钮作为既有坐标内的导入入口，并完成搜索、置顶、复制、远程只读导出、显式刷新、成功后删除源文件和 Shift 二次点击删除；Minecraft Java JSON 与受支持 Blockbench `.bbmodel` 导入；路径穿越、符号链接逃逸、超大/过深文件和非法元素防护。

**验收清单**：普通磁盘不能放入，只有结构磁盘可用；无结构磁盘时加载/存入按钮禁用，点击共享模型行只保留选中态并提示放入结构磁盘；放入结构磁盘后点击模型行只写盘、不改成型舱，点击“加载磁盘”才把受支持模型送入成型舱；空结构磁盘一次点击存入，非空结构磁盘标题栏提示后再次点击覆盖，未锁定或锁定状态都可存入且失败不损坏原数据；普通方块结构磁盘继续显示本体 Tooltip、智能方块放置器判定和方块预览，成型蓝图结构磁盘改为显示模型名、真实三轴尺寸、成型舱加工判定及同速旋转模型预览；超出 `48x48x48 px` 的模型可导入共享库、写入结构磁盘并载入可编辑建模区，但锁定按钮保持禁用；建模区有 cube 时加载模型先在标题栏提示、再次点击清空并导入；载入后不锁定且可编辑；8 B 暂存罐非空时仍可载入且流体不变，只有成型区域有批次熔体时才拒绝覆盖；叠加层每次打开 UI 默认关闭并且只由模型管理按钮切换；列表只绘制实际模型按钮，长名水平滚动、长列表纵向滚动，右侧滑块使用 `slider.png` 的标准/悬浮两个 `4x12` 半幅且轨道按规定调整，列表不足七项时保持标准帧且没有禁用灰层；没有选中模型时仍能按住 Shift 打开目录，放入白名单文件后不会后台自动导入，普通点击刷新才扫描；服务端导入成功后删除未变化的源文件，失败或源文件已变化时保留；需要整体平移时再次普通点击刷新确认；Blockbench `(0,0,0)` 映射到成型舱 `(0,0,0)` 且 `48x48x48` 模型可识别；置顶不改其他玩家排序；存入/导入/复制的 owner 正确；远程服务器只打开本地只读导出副本且该副本不回传；删除与重名复核权限和修订号；结构磁盘在 JSON 丢失后仍可载入；不支持的模型元素列出具体错误而不静默丢形状。

**本项不包含**：生产锁定和塑料加工。

### TODO-03 供电、粘土模具、熔体缓冲和自动化状态机

**玩家可见结果**：玩家可以在任何机器阶段从顶部或底部管道或资源槽向 8 B 暂存罐输入塑料熔体，也可用资源槽中的电容器给成型舱供电、用多相超限合金打破类型限制并投入粘土球；区域无实体时，粘土逐层动画与完整碰撞同时开始，模具完成后机器以固定 `250 mB/gt` 把暂存熔体泵入成型区域。

**前置依赖**：TODO-01。

**规格范围**：第 2.2 节、第 3.3 节的机器控件、第 4.2、4.3 节、第 5 节和第 11 节。

**必须交付**：`256 kW` AnvilCraft 用电接入、等同一个超级电容器的内部 FE 储能和第 3.3 节 Jade 同款红色分格 FE 条；固定只收粘土球、按溜槽样式渲染并用滚轮在 `1..256` 调整和持久化堆叠上限的单过滤槽与物品能力，客户端菜单同步同样不得截断到 64；接收塑料熔体桶和已充电电容器、每 gt 最多处理一个并正确返还空容器的资源槽；粘土需求计算、三层整批收料成模、暂停/恢复/解锁退还/破坏掉落；`12 gt` 填充动画每 `4 gt` 一次完成整层、暂停时冻结、填充期间保留模型投影、动画启动前一次实体占用检查、阻塞 GUI 提示、启动事务内立即启用的 3x3x3 完整碰撞和完整粘土纹理；顶面与底面固定共用的双向流体能力、GUI 中菜单光标所持塑料熔体桶/空桶右击带 Tint 且按 `4 B/16 px` 原尺寸裁剪平铺的纹理储量条服务端事务、任何阶段均可外部输入的独立 8 B 暂存罐、锁定且模具完成后固定 `250 mB/gt` 向按 `1 mB = 4` 立方像素计算的独立 `M(V)` 成型区域泵送、反向抽出、解锁时空间足够则整批自动回抽、流体身份/颜色一致性和成型区域最少 250 mB；三种互斥模式、红石上升沿锁存、持续高电平去重、区域/缺料/供电阻塞恢复；8 B 暂存与成型区域熔体的独立持久化及破坏时舍弃暂存熔体；为 TODO-05 提供只结算成型区域熔体的原子加工快照和成功/失败状态转移。

**验收清单**：粘土槽使用溜槽同款虚影、红色覆盖和上限数字，滚轮上限不能小于 1 或大于 256、默认 256、手动左键放入、Shift 快捷放入、菜单同步、重开与区块重载均实际保留最多 256 球，设为 1 时物流不能插入第 2 球且过滤始终不能换成其他物品；玩家主动设为 64 时可由溜槽分批补到理论最多 108 球；资源槽可处理同色同种塑料熔体桶和已充电电容器，每 gt 最多一个，空容器按槽位、玩家背包、地面顺序返还；实体阻塞时动画不开始且 GUI 明确提示，移走后只检查一次并在动画第一帧启用完整碰撞，动画末层不做第二次占用检查；模型投影在填充期间继续显示，第 4/8/12 gt 仅在本层所需粘土可一次性扣足时各出现完整 3x3 一层，缺料/失电时碰撞、进度和整层渲染都冻结且不插值抖动；顶部与底部流体能力在空模型、编辑、断电、未成模、加工和等待状态均可向 8 B 暂存罐输入并按既定顺序输出，水平面不暴露该能力；菜单光标持同色同种塑料熔体桶可从 GUI 倒入、空桶可舀出，主手不参与，异色/异种或容量不足时桶与流体均不变；GUI 熔体按 `4 B` 一张完整 `16x16` Tint 流体纹理裁剪平铺，`1 B` 只显示底部 `1/4`，FE 按 `7x30` 纵向内容区由下向上按 Jade 红色分格样式绘制；泵送只有锁定且模具完成并有电时发生，实测速率每 gt 不超过且能达到 250 mB；暂存罐和成型区域可同时达到 `8000 mB + M(V)`，最大模型区域可累计 `27648 mB`，两处合计 `35648 mB`；批次熔体能够完整放回暂存罐时解锁自动回抽，空间不足时拒绝且两边数量不变；破坏机器不掉落或保留 8 B 暂存熔体；默认红石模式只响应一次上升沿；单次模式忽略红石；区块重载不重复取粘土、泵送或丢熔体。

**本项不包含**：巨型铁砧产出成品、成品渲染/碰撞和功能箱罐；加工成功后的粘土实际掉落事务在 TODO-05 与制品产出一起接入。

### TODO-04 24 朝向动态制品、自动碰撞和物理机制

**玩家可见结果**：任意已制造形状都能成为可持有、放置、回收和参与现有 Plasticraft 物理的动态塑料制品；它与原有 `16x16x16` 通用塑料块的行为完全一致，只替换为模型实际包含的源 cube 外观和对应碰撞。实体可处于 24 种姿态，但物品始终使用默认方向且不显示额外放置预览。

**前置依赖**：TODO-01。贴图接入必须直接复用仓库现有的 `dev.anvilcraft.plasticraft.api.texture` 公共 API，不得在本 TODO 内另做一套自由制品专用生成器。

**规格范围**：第 7 节，并复用第 4.1 节的派生形状；第 7.3 节在本 TODO 只交付共用 Tooltip 入口和未指定/普通制品映射，箱子、储罐、铁砧与支架映射归 TODO-06。
...
5. 让普通与内部调试覆盖产生的超限制品都继承现有塑料实体的重力、浮力、动态引力、推动、滑轨、磁化、粘合、持久化、铁砧锤和回收机制，并移除这些系统中的固定 1x1x1 与 3x3x3 尺寸假设。
...
**本项不包含**：功能类型的专用行为；普通制品完整生产链必须在本 TODO 结束时已可用。

### TODO-06 功能类型、铁砧、支架、无 GUI 箱子/多流体储罐和动态液面

**玩家可见结果**：玩家可点击主界面左侧“指定类型”打开类型选择叠加层，选择箱子、储罐、铁砧或支架，由服务器根据实际形状判定是否合格并计算适用容量。加工后的箱子/储罐没有自己的 GUI，但可被物流或流体系统存取，并在 Jade/铁砧锤浮窗显示内容；铁砧成品可打开独立铁砧 GUI 并执行落砧配方，只有局部下底面朝下撞击才触发；复杂储罐中的多种流体随动态重力形成体积正确的斜液面；支架可放入五种受支持红石元件，并在移动、离网格和全部 24 姿态下保留交互与六向红石能力。

**前置依赖**：TODO-04 和 TODO-05。

**规格范围**：第 3.4 节、第 7.3 节的全部功能类型映射、第 8 节和第 9 节。

**必须交付**：可扩展类型注册表、普通/箱子/储罐/铁砧/支架稳定 ID、左侧“指定类型”按钮与可搜索滚动的类型叠加层、当前选择与失败提示；箱子/储罐的 padding 六邻接外壳与内腔洪水扫描、零厚度 cube 阻隔面、多腔求和、内部装饰拒绝和 `C/(C+S) >= 0.5`；铁砧固定世界 Y 轴的最低层平底扫描、`12x12 px` 下底面、`3/3/4 px` 三段厚度、顶部严格厚于底部、中段四边严格内缩和顶部投影面积更大规则；支架固定世界 Y 轴、中心 `16x16 px`、最多 `4 px` 高、顶底投影相同、底面四邻接无孔、中心 `2x2 px` 完整和真实水平顶底面的扫描；箱子每 64 内腔单元一格、储罐每 171 内腔单元 1 B 的容量派生；第 7.3 节由最终类型和派生容量驱动的全部类型 Tooltip，以及箱子/储罐物品位于指针正上方的原版边框独立内容浮窗；铁砧成品的落砧事件、默认 HasAnvil 谓词和共用铁砧 GUI；支架对五种红石元件的放入、取回、原模型渲染、原交互、持久化、24 姿态和移动离网格六向红石运行时；部分成型重新验证和降级普通制品；无菜单任意容量物品能力；与 AnvilCraft 大型流体储罐同语义的无菜单共享容量多流体能力；内容随物品/实体往返保存与安全破坏排出；Jade/铁砧锤浮窗分页/汇总；任意重力向量下的内腔网格、等体积半空间裁切、多流体分层、透明排序、重力插值和拓扑缓存；四种塑料铁砧各自的灰白着色底图与紫黑非着色覆盖图占位资源。

**验收清单**：类型叠加层默认关闭且只由“指定类型”按钮切换，搜索、滚动和选中态正确，原版铁砧图标出现在铁砧行，支架行暂用标准通用塑料块图标，选择无效箱子/储罐/铁砧/支架时保留编辑并禁止锁定；成品物品和实体按“塑料材料名 + 最终类型后缀”命名且不显示模型名；铁砧形状边界覆盖 `11x12` 失败、总高 `9 px` 失败、底/中/顶厚度 `2/3/5` 失败、中段投影有一边相等失败、顶部投影面积相等失败、顶部不比底部厚失败，以及底部上层轮廓略外扩但规则仍满足的通过案例；支架覆盖合法中心模型、超过 `4 px`、越出中心一格、中心 `2x2 px` 缺失、底面断连或有孔、顶底投影不同、斜底/斜顶、零厚度底和部分成型降级；完整铁砧成品只有下底面朝世界下方撞击才发布一次 `AnvilEvent.OnLand`，侧面/顶面/倒置不发布；默认 `HasAnvil` 可匹配而专用属性或 NBT 谓词不误匹配，实体和粘附方块态都能打开铁砧 GUI；零厚度 cube 可密封但无碰撞；多腔求和且内部装饰导致验证失败；箱子 `1728 -> 27`、`63 -> 失败`、`64 -> 1`，对应物品分别显示容量参数 `27` 和 `1` 的箱子 Tooltip；储罐 `2744 -> 16 B`、`170 -> 失败`、`171 -> 1 B`，对应物品分别显示容量参数 `16` 和 `1` 的储罐 Tooltip；容量不设额外上限且 Tooltip 不随当前内容变化；箱子/储罐物品的内容浮窗独立显示在指针正上方并使用原版默认 Tooltip 边框，两种类型的摘要首行为白色，其余逐项内容为淡灰色，全部文字均带默认深灰色阴影；Jade 储罐只显示默认流体条，铁砧锤逐项内容文字为淡灰色并带深灰色阴影；大容量浮窗不爆包；多种流体的 ID、组件、数量和顺序完整往返；0°/30°/45°/90° 和连续变化的重力下液体体积不变；凹形、细管、多腔和零厚度 cube 密封模型不穿模、不留非预期空洞、流体层不重叠；五种支架元件白名单、配置 Codec/网络往返、右击放入与潜行空手取回、原交互和模型渲染、物品探测器 `range=1`、动态与粘合状态、离网格位置及全部六个安装方向均有回归验证。

**本项不包含**：支架正式专用图标美术；当前复用普通模式的 `16x16x16 px` 标准通用塑料块。
...
1. 连续凸碰撞不再是成型舱动态制品的特例。`UniversalPlasticEntity` 的默认 `16x16x16` 模型、`ResinAnvilEntity`、`HardenedResinAnvilEntity`、`HardenedResinCauldronEntity`、`CatalyticPressLidEntity` 以及以后新增的全部塑料实体都必须走同一套模型 Cube、朝向缓存、宽相位和 SAT 窄相位逻辑。
...
10. 通用塑料 Tooltip 已显示三轴实际尺寸：默认方块为 `1 x 1 x 1`，动态制品使用真实表面尺寸并最多保留四位必要小数。英文 DataGen 源、英文生成语言、中文语言和中英文 Ageratum 手册均已同步，`runData` 已成功执行。
...
2. 轮廓生成现由 AnvilLib 520 的 `OutlineBuilder` 负责：收集源棱线与非共面面相交线，切分后排除内部棱线与共面接缝，再合并共线段。实体、方块态与 F3+B 共用 `OutlineCache`，首次查询返回轻量轮廓，后台在时间、工作量与线段预算内生成并集棱线。
3. `PlasticSelectionGeometry` 按不可变源几何建立弱键、限重的客户端缓存，同一模型的 24 种朝向与粘合定位只改变 `SelectionPart` 的矩阵，不按世界位置重算并集。旧的碰撞箱轮廓缓存已删除。合并完成后，两个相邻单位 Cube 仍得到长方体的 `12` 条棱线，单个旋转 Cube 仍保留 `12` 条斜棱。
4. 准星实体候选与方块化提供器使用同一份砧库 BVH。零厚度面和托盘附加交互部件独立保留，不把旋转凸体的粗略外接盒加入精确选择几何；F3+B 仍只显示物理凸体。局部缓存坐标放大后由位姿还原，以绕开 520 对极薄面法线的归一化容差，不改变实体物理几何。超出砧库邻格选择范围的巨型方块态保留兼容选取，并继续使用砧库轮廓绘制。
