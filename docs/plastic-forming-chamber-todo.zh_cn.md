...
1. 成型制品的物品与实体显示名由塑料材料本地化名称和最终功能类型后缀共同组成，不使用蓝图或模型名称。普通类型后缀为“块”，其他类型默认复用类型注册项的本地化名称；例如通用塑料依次命名为“通用塑料块”“通用塑料箱子”“通用塑料储罐”“通用塑料铁砧”“通用塑料支架”。未来新增塑料材料和功能类型继续使用同一组合规则。
...
4. 文案中的 `%s` 是本地化格式参数，不是需要原样显示的字符。箱子传入第 8.3 节派生的物品格数，储罐传入第 8.4 节派生的整数 `B` 容量；两者都显示制品的总容量，不随当前已存内容变化。现有未指定类型的通用塑料块继续使用同一句 `一块普普通通的塑料`，不得另造普通制品文案。
5. 铁砧 Tooltip 不带容量参数，只说明其落砧配方能力；支架 Tooltip 不带容量参数，完整列出五种可承载元件。部分成型若不再满足对应验证器则降级为普通制品。箱子和储罐物品在原有基础 Tooltip 之外，还要在指针正上方显示独立的内容浮窗；该浮窗使用原版默认 Tooltip 边框，不使用药盒或护符盒的专用贴图。箱子首行“已用格数”和储罐首行“已有流体”为白色，其余逐项内容为淡灰色，所有文字均保留默认深灰色阴影。箱子当前内容也由 Jade 和铁砧锤浮窗展示；储罐在 Jade 中只使用其流体能力提供的默认流体条，铁砧锤浮窗仍显示流体总量和逐项内容。铁砧锤浮窗的物品、流体逐项内容文字同样使用淡灰色并保留深灰色阴影。
