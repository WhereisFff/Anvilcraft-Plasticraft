...
| 通用塑料铁砧 GUI 着色底图 | 占位实现 | `assets/anvilcraftplasticraft/textures/gui/background/universal_plastic_anvil_base.png`，`176x166` | 灰白方块占位纹理，按通用塑料颜色着色 |
| 通用塑料铁砧 GUI 非着色覆盖图 | 占位实现 | `assets/anvilcraftplasticraft/textures/gui/background/universal_plastic_anvil_overlay.png`，`176x166` | 紫黑方块占位纹理，不按塑料颜色着色 |
...
2. 叠加层内部继续使用左上角为 `(0,0)` 的半开矩形坐标：
   ...
   每行最左侧 `15x15` 范围内按 `[6, 20+15r, 10, 10]` 渲染图标；该区域位于原中央 `11x11` 范围的左上角，即只从右侧和底部各缩小 `1 px`。名称从该 `15x15` 范围之后开始且位置不变。`r=0..4` 对应表中五行并全部实际填充，铁砧行使用原版铁砧图标；支架正式图标完成前复用普通类型的白色 `16x16x14 px` 标准通用塑料块。未来类型继续由注册表生成并使用滚动列表。
3. 搜索框只有被鼠标点击并取得焦点后才接收键盘输入；未取得焦点时，玩家键入内容不得进入搜索。取得焦点后每次文本变化都实时更新可见类型，不需要按回车；关闭叠加层、按 Esc 或点击界面内任何不属于搜索框的位置都立即结束输入。
4. 每个类型行使用 `category_entry.png` 的三段按钮贴图，依次为标准、鼠标悬浮、选中。类型选择是单选；点击一项会松开原选中项并选中新项。行左侧图标区独立渲染对应类型的立体物品图标；普通类型使用白色 `16x16x14 px` 基础通用塑料块，箱子使用原版箱子，储罐使用 AnvilCraft 流体储罐，铁砧使用原版铁砧，支架暂时与普通类型使用同一标准通用塑料块。主界面“指定类型”按钮也在四态背景之上独立渲染同一类型图标，类型图标不占用按钮背景帧。
...
5. 大容量浮窗必须分页/滚动或按相同物品汇总，不能一次发送/绘制全部空槽。实体回收为物品时保留库存，并防止递归容器数据造成无限嵌套。内部调试覆盖产生的超大容器在 1.21.1 暂时允许较大的 NBT；迁移优化见 `docs/molding-26.1-storage-migration.zh_cn.md`。
...
2. 最低层必须包含连续、无孔的至少 `14x14 px` 平整矩形，并保持整个模型沿 Y 连续；相邻占用层必须有重叠，不能只在角上接触。
...
7. GUI 使用通用塑料铁砧独立的两层 `176x166` 纹理：`universal_plastic_anvil_base.png` 是按材料颜色着色的灰白方块占位图，`universal_plastic_anvil_overlay.png` 是不着色的紫黑方块占位图；未来材料继续沿用一对基础图和覆盖图。
...
9. 支架类型行和主按钮的正式图标尚未完成；当前固定复用普通模式的白色 `16x16x14 px` 标准通用塑料块，后续只替换图标资源，不改变类型 ID、行坐标或行为。
