...
- **`MoldedPlasticAnvilGameTests`**：成型铁砧谓词与菜单、落地朝向、巨型多方块与冲击。成型锅单次撞击的落砧配方通道数（普通 1 次、大型 9 次）也在本类：断言产出与剩余输入的数量，不要断言配方管线的中间值。
...
- **`AllayGameTests` / `AllayLoungeGameTests`**：戴帽转换、空手通用工、切石机只拆除、磁铁不建设、空闲走原版游荡而任务中不乱跑、推挤拴绳与无硬碰撞，以及迎面避让不摆动、单格通道无避让空间时互相穿过；休息室召回节流、稳定环形等待队列、满员拒绝、破坏放出、通道忙时 GUI 放出失败、室内暂停/跳过与收集出库优先磁铁。休息室与任务已有窄接口物流测试，仍不要复制四工种现场作业。断言出库通道按 20 gt 释放时必须自己清掉刚出库的那只悦灵：任何无活可做的悦灵都会自行返库再占一次通道，留着它测到的就不是通道本身。不要再写无人机组装、内部 FE、电量拒派、休息室耗电或 5×5×5 游荡。
