...
- **`ConstructionJobGameTests`**：单架无站建设/拆除无人机的施工闭环窄接口。台账扣料与无实体时的取消当场返还、已取料无人机停止后飞回还物、已交付假方块碰撞/占用/空碰撞、安静提交与部分取消、缺料暂停与跳过残缺、电量不足拒派、同模板短时序交付，以及流体封堵、切石机砸击拆除、拆除不写已交付投影、永久障碍跳过、缺拆除策略，还有任务掉落只收标记物、玩家捡起外部结算不补发、无收集机不阻塞建造、有收集机时有界清场以及清场后离场再降落。方块实体内容真实扣料、多方块只耗核心、邻接投影碰撞、红石稳定复原和分区提交日志也放在本类 `zzz_construction`，不要并进拆除或收集 live。桶装流体、非整桶储罐、船/刷怪蛋/树脂块与树脂铁砧捕获/塑料实体与封闭实体接近位与瞬态跳过放在本类 `zzz_construction_adapt`，仍不要并进拆除或收集 live。不要复制进 `DroneGameTests`、`BlueprintConstructionGameTests` 或 `CollectionDroneGameTests`，也不要 `inflate(128)` 扫描实体。空气建造、拆除、封堵与收集清场的失败条件不同，必须分开；瞬时断言与短时序飞行分属不同 GameTest batch，避免与蓝图索引生命周期测试并行共享世界级任务 SavedData，也避免短时序飞行与 20 gt 断言抢同一批超时窗口。
- **`CollectionDroneGameTests`**：收集无人机自由拾取的窄接口，不启动施工任务以免污染施工 SavedData。覆盖九格插入与堆叠、满载拒收、16 格内可选而格外不预锁、向所有者卸货时剩余留机，以及短时序飞到 16 格内吸入。瞬时断言走 `zzz_collection`，飞行走 `zzz_collection_live`，不要并进拆除或空气墙 live batch。
