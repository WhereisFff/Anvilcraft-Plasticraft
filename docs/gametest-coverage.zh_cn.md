...
- **`ConstructionJobGameTests`**：单架无站建设/拆除无人机的施工闭环窄接口。台账扣料与无实体时的取消当场返还、已取料无人机停止后飞回还物、已交付假方块碰撞/占用/空碰撞、安静提交与部分取消、缺料暂停与跳过残缺、电量不足拒派、同模板短时序交付，以及流体封堵、切石机砸击拆除、永久障碍跳过和缺拆除策略。不要复制进 `DroneGameTests` 或 `BlueprintConstructionGameTests`，也不要 `inflate(128)` 扫描实体。空气建造、拆除与封堵的失败条件不同，必须分开；瞬时断言与短时序飞行分属不同 GameTest batch，避免与蓝图索引生命周期测试并行共享世界级任务 SavedData，也避免短时序飞行与 20 gt 断言抢同一批超时窗口。
